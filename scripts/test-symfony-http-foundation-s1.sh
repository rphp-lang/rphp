...
fixture_source="${repository_root}/tests/fixtures/symfony-http-foundation"
...
fixture="$(mktemp -d "${TMPDIR:-/tmp}/rphp-symfony-http-foundation-s1.XXXXXX")"
...
expected='201|http-foundation|POST|/hello|RPHP|value|alpha'
reference="$(php "${fixture}/run.php")"
if [[ "${reference}" != "${expected}" ]]; then
    printf 'error: reference PHP HttpFoundation output mismatch\nexpected: %s\nactual:   %s\n' \
...
if [[ "${actual}" != "${reference}" ]]; then
    printf 'error: Symfony HttpFoundation S1 gate mismatch\nreference: %s\nRPHP:      %s\n' \
        "${reference}" "${actual}" >&2
    exit 1
fi

echo "Symfony HttpFoundation v7.4.16 S1 gate passed: ${actual}"
