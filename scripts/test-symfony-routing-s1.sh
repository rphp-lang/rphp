...
fixture_source="${repository_root}/tests/fixtures/symfony-routing"
...
fixture="$(mktemp -d "${TMPDIR:-/tmp}/rphp-symfony-routing-s1.XXXXXX")"
...
# A developer may have an ignored local vendor tree. Never let it mask the
# locked clean install that this reproducibility gate is meant to verify.
...
expected='health|article_show:rphp-8:html|405:GET,HEAD|404'
reference="$(php "${fixture}/run.php")"
if [[ "${reference}" != "${expected}" ]]; then
    printf 'error: reference PHP Routing output mismatch\nexpected: %s\nactual:   %s\n' \
...
if [[ "${actual}" != "${reference}" ]]; then
    printf 'error: Symfony Routing S1 gate mismatch\nreference: %s\nRPHP:      %s\n' \
        "${reference}" "${actual}" >&2
    exit 1
fi

echo "Symfony Routing v7.4.15 S1 gate passed: ${actual}"
