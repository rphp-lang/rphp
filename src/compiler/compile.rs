...
pub struct CatchEntry {
    ...
    pub line: u32,          // source line of the catch boundary
...
fn builtin_ref_args(name: &str) -> u64 {
    ...
    match name {
        ...
        "preg_replace" | "preg_replace_callback" | "preg_filter" => 0b1_0000, // arg 4 (&$count)
        "preg_replace_callback_array" => 0b1000,  // arg 3 (&$count)
...
impl Compiler {
    ...
    fn validate_deprecated_target(
        ...
        if !allowed && !self.has_delayed_target_validation(attributes) {
            return Err(self.goto_error(
                &format!(
                    "Attribute \"Deprecated\" cannot target {target} (allowed targets: class, function, method, class constant, constant)"
                ),
                first_line,
            ));
        }
        if let Some(line) = lines.next() {
            return Err(self.goto_error("Attribute \"Deprecated\" must not be repeated", line));
        }
        Ok(())
    }

    fn validate_deprecated_class_form(
        &self,
        attributes: &[Attribute],
        target: &str,
    ) -> Result<(), String> {
        let Some(line) = self.deprecated_attribute_line(attributes) else {
            return Ok(());
        };
        if self.has_delayed_target_validation(attributes) {
            return Ok(());
        }
        Err(self.goto_error(&format!("Cannot apply #[\\Deprecated] to {target}"), line))
    }
        ...
        match expr {
            ...
            Expr::CoalesceAssign { target, expr } => {
                match self.compile_coalesce_assign_expression(target, expr) {
                    Ok((result, result_type, _)) => (result, result_type),
                    Err(error) => {
                        self.deferred_error = Some(error);
                        let null = self.add_literal(Value::null());
                        (null, OpType::Const)
            ...
            Expr::ArrayAccess { array, index, line } => {
                ...
                let index_may_diagnose_undefined = matches!(index.as_ref(),
                    Expr::Variable { name, line }
                        if *line != 0
                ...
                if index_may_diagnose_undefined && arr_type == OpType::Cv {
                    // PHP evaluates the container before the dimension key.
                    // A maybe-undefined key can enter user code and replace
                    // that CV, but this read still consumes the pre-handler
                    // container snapshot.
                    let snapshot = self.alloc_tmp();
                    let mut retain = Instruction::new(OpCode::AssignCv);
                    retain.op1 = snapshot;
                    retain.op1_type = OpType::Tmp;
                    retain.op2 = arr_op;
                    retain.op2_type = OpType::Cv;
                    self.instructions.push(retain);
                    arr_op = snapshot;
                    arr_type = OpType::Tmp;
                }
            ...
            Expr::ErrorSuppress(inner) => {
                ...
                for instruction in &mut self.instructions[first_instruction..] {
                    ...
                    } else if instruction.opcode == OpCode::AssignObjProp {
                        instruction._pad |= ASSIGN_OBJ_ERROR_SUPPRESS;
                    } else if instruction.opcode == OpCode::AssignDim {
                        instruction._pad |= ASSIGN_DIM_ERROR_SUPPRESS;
    ...
    fn emit_constructor_argument_release(&mut self, start: u16, end: u32, line: usize) {
        ...
        release.op2_type = OpType::Tmp;
        release._pad |= RELEASE_TEMPS_SUBEXPRESSION | RELEASE_TEMPS_CONSTRUCTOR_ARGUMENTS;
        self.push_instruction_at_line(release, line);
    }
