...
use super::instruction::{
    ARITHMETIC_COMPOUND_ASSIGN, ARITHMETIC_NEXT_PRIMITIVE_ASSIGN,
    ARRAY_ELEMENT_COMPOUND_APPEND_WRITEBACK, ARRAY_ELEMENT_DEFER_NONREFERENCEABLE_NOTICE,
    ...
    ASSIGN_CV_MOVE_SOURCE, ASSIGN_CV_REBIND, ASSIGN_DIM_DIAGNOSTIC_GUARD,
    ...
    CLASS_CONST_DYNAMIC_OWNER, CLONE_OBJ_WITH_PROPERTIES, FETCH_DIM_COMPOUND,
    ...
    FETCH_OBJ_UNSET, INSTANCEOF_DYNAMIC_STATIC_SCOPE, Instruction, JMP_NZ_RELEASE_TEMPS,
    KnownScalarType, LATE_STATIC_PROP_EMBEDDED_SCOPE, NEW_FLAG_DYNAMIC_CLASS_NAME,
    NEW_FLAG_DYNAMIC_STATIC_SCOPE, NEW_FLAG_PREPARE_ONLY, NEW_FLAG_PREPARED,
    NEW_FLAG_UNPACKED_ARGUMENTS, NEW_FLAG_UNRESOLVED_LEXICAL_SCOPE, NEW_FLAG_VALIDATE_ONLY,
    NEW_FLAG_VIRTUAL_DECLARED_READS, NEW_FLAG_VIRTUAL_OBJECT_ARRAY_PIPELINE, OBJ_PROP_FUNC_ARG,
    OBJ_PROP_REFERENCE_BIND, OBJ_PROP_TEMPORARY_RECEIVER, OpType, PROPERTY_INCDEC_DECREMENT,
    ...
    SEND_FLAG_NONREFERENCEABLE, SEND_FLAG_PREPARED_PROPERTY_ARGUMENT, SEND_FLAG_YIELD_SNAPSHOT,
...
#[derive(Clone, Copy)]
struct InternalCallContext {
    caller: *mut ExecuteData,
    user_instruction: Option<(OpCode, u16)>,
}

/// Recover the consuming user instruction for a synchronous internal handler.
/// Internal-to-internal callers and detached activations have no such site.
pub(crate) fn instruction_for_internal_call(
    internal_frame: *mut ExecuteData,
) -> Option<(OpCode, u16)> {
    internal_call_context(internal_frame)?.user_instruction
}

fn internal_call_context(internal_frame: *mut ExecuteData) -> Option<InternalCallContext> {
    if internal_frame.is_null() {
        return None;
    }
    // SAFETY: dereferencing the saved caller is valid because an internal
    // handler and its immutable consuming instruction remain synchronously
    // live for this lookup. Only compact opcode metadata escapes it.
    unsafe {
        let caller = (*internal_frame).prev_execute_data;
        if caller.is_null() {
            return None;
        }
        let user_instruction = if !(*caller).func.is_null()
            && Function::from_common_ptr((*caller).func).fn_type() == FunctionType::User
            && !(*caller).opline.is_null()
        {
            let instruction = &*(*caller).opline;
            Some((instruction.opcode, instruction._pad))
        } else {
            None
        };
...
fn execute_full_call<'a>(
    ...
    let original_user_arguments = (func_common.fn_type == FunctionType::User
        ...
        .then(|| {
            ...
            let mut arguments = eg.take_function_argument_buffer(extra_count as usize);
