...
fn solar_events_utc_hours(
    ...
    if !(-1.0..=1.0).contains(&cosine_hour) {
        return None;
    }
    let hours = cosine_hour.acos().to_degrees() / 15.0;
    Some((
        (transit - hours).rem_euclid(24.0),
        (transit + hours).rem_euclid(24.0),
        transit,
    ))
}
...
fn solar_transit(
    ...
    let (year, month, day, _, _, _, _, _) =
        super::super::unix_to_parts(timestamp.saturating_add(offset));
    let transit = solar_events_utc_hours(year, month, day, latitude, longitude, 90.583333)?.2;
    let midnight = super::super::parts_to_unix(year, month, day, 0, 0, 0);
    Some(midnight.saturating_add((transit * 3_600.0).round() as i64))
}
...
fn sun_value(
    ...
    let Some((event, utc_hours)) = solar_event(timestamp, latitude, longitude, zenith, sunrise, eg)
    else {
    ...
    match return_format {
        ...
        1 => {
            ...
            let total_minutes = (local * 60.0).round() as i64;
            Value::string(format!(
                "{:02}:{:02}",
...
pub(crate) fn fn_date_sun_info(
    ...
    for (name, zenith, sunrise) in [
        ...
        let value =
            solar_event(timestamp, latitude, longitude, zenith, sunrise, eg).map(|event| event.0);
        events.push((name, value));
    }
    result.set_str(
        "sunrise",
        events[0].1.map_or_else(|| Value::bool(false), Value::long),
    );
    result.set_str(
        "sunset",
        events[1].1.map_or_else(|| Value::bool(false), Value::long),
    );
    result.set_str(
        "transit",
    ...
    for (name, value) in events.into_iter().skip(2) {
        result.set_str(name, value.map_or_else(|| Value::bool(false), Value::long));
    }
...
mod tests {
    ...
    fn solar_equator_events_are_finite_and_ordered() {
        let (rise, set, _) = solar_events_utc_hours(2024, 3, 20, 0.0, 0.0, 90.833333).unwrap();
        assert!(rise < set);
    }
}
