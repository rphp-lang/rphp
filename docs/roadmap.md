...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is 2,439/5,599 with zero crashes or timeouts; trait consumers and aliases own independent function-static cells, and `global`/`static` CV rebinding preserves the global value. Closure storage ownership and destructor timing remain next. |
