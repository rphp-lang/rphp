...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,202/5,599 with zero XFAIL, timeout or crash. The latest checkpoint closes all 10 ordinary `get_html_translation_table_basic*` cases through reproducible W3C/WHATWG HTML4/HTML5 tables, PHP-compatible flags, encoding filters and raw-byte legacy keys. Strings reaches 418/733 (+12/-0), with 230 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. CPU-pinned paired medians stay below the +5% gate for ASCII offsets (+0.618%), `asort()` (+0.821%) and keyed `foreach` (+2.217%), while startup is -3.575%. Broader named-entity encoding/decoding, 32-bit execution, remaining strings/Zend gaps and wider PHP compatibility remain explicit work. |
