...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,436/5,599 with zero timeouts and zero crashes. No-argument `get_class()` emits PHP 8.5's handler-aware deprecation inside class scope before returning the lexical declaring class, while outside-scope errors and explicit-object calls remain unchanged. The exact delta is +3/-0 over `099be260`, with two byte-identical full manifests and no other category movement; every supported upstream `class_alias` case is now exact. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; no hot-path performance gate applies to the isolated deprecated branch. Remaining CLI capability, eval-diagnostic and other PHP 8.5 clusters stay explicit compatibility work. |
