...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,433/5,599 with zero timeouts and zero crashes. `class_alias()` conflicts emit the canonical `Cannot redeclare` warning with the source symbol's kind, raw alias and user declaration origin while internal sources omit the unavailable origin. The exact delta is +4/-0 over `6592be1b`, with two byte-identical full manifests and no other category movement. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; the successful alias path is unchanged, so no hot-path performance gate applies. The remaining alias deprecation output, eval diagnostics and other PHP 8.5 clusters stay explicit compatibility work. |
