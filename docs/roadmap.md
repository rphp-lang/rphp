...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus moves from 4,103 to 4,107/5,599 (+4/-0) with zero timeouts or crashes and two byte-identical manifests. `array_push()`, `array_pop()`, `array_shift()`, `array_unshift()` and `array_splice()` now cover exercised variadic, key, bound, replacement, reference/COW, cursor, destructor, reentrant-error and live by-reference `foreach` mutation behavior; negative-first-key append history also matches PHP 8.5. Their 59-case focused cluster moves from 22 to 50 passes (+28/-0), and the complete 842-case array audit moves from 496 to 525 (+29/-0), both without a lost pass. Five original E2E tests, the clean-room oracle, all five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Paired startup/append/mutator medians range from -4.857% to +2.926%, below the +5% gate. Nine focused parser/notice/output/fatal-envelope gaps, general by-reference write-context enforcement, complete reflected internal metadata, near-limit allocation and broader compatibility remain explicit work. |
