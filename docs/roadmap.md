...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set is 4,117/5,599 (+6/-0) with zero timeouts or crashes. Function and method Reflection now includes invocation, named and variadic argument behavior, short/namespace names, closure names and closure scope metadata; its 14-case focus moves from 1 to 12 passes (+11/-0), the complete 493-case Reflection audit from 78 to 89 (+11/-0), and the adjacent closure/dynamic-call audit from 95 to 100 (+5/-0), without a lost pass. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. After rejecting a generic path with a ~96% method-control regression, the accepted narrow fast path puts four 32-pair controls between -0.258% and +1.162% paired, below the +5% gate. `call_user_func()` unknown named forwarding, `DateTime`, complete Reflection metadata and broader compatibility remain explicit work. |
