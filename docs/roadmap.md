...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,423/5,599 with zero timeouts and zero crashes. A shared terminal-name rule makes reserved class-like declarations, class imports, runtime aliases and `_` deprecations exact for +27/-0 over `91133161`; all five feature configurations, unsafe ratchet, Composer S0, Symfony S1/S2/S3 gates and the CPU-pinned cold-declaration control pass. Eval compile-error wrapping, internal-class alias output and the remaining PHP 8.5 clusters stay explicit compatibility work. |
