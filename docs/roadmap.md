...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,202/5,599 with zero XFAIL, timeout or crash. The latest checkpoint adds the PHP-byte contracts for `addslashes()`, `stripslashes()`, `addcslashes()` and `stripcslashes()`, including ranges, C/octal/hex decoding and strict/weak diagnostics. Strings reaches 450/733 (+14/-0), with 198 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Exact-binary CPU-pinned paired medians are -5.621% for `addcslashes()`, +0.296% for concatenation and +0.391% for startup, below the +5% gate. The 11-case `trim()`/`ltrim()`/`rtrim()`/`chop()` byte-charlist cluster is next; typed-parameter byte provenance, 32-bit execution, remaining strings/Zend gaps and wider PHP compatibility remain explicit work. |
