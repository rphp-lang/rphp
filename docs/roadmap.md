...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,992/5,599 with zero timeouts and zero crashes. False write destinations now publish an array and emit PHP 8.5's reentrant deprecation across indexed, append, nested, reference, property, destructuring, foreach and compound forms; handler replacement wins without later dimension evaluation or stale writeback. Compound and foreach terminal `[]` targets are admitted. The exact delta is +4/-0 over `67e45bcd`, with two byte-identical full manifests; four remaining failures advance to documented independent boundaries and no other category moves. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; paired release ratios are 0.996750 for empty requests, 0.983890 for direct array writes and 0.961226 for nested writes, below the +5% gate. Throwing-handler commit timing, effectful direct keys, typed boolean properties, by-reference foreach teardown, string append diagnostics and other PHP 8.5 clusters stay explicit work. |
