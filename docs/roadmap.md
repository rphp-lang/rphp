...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 positional-format checkpoint reaches 617/733 strings, exactly +1/-0 from `bug69751.phpt`, while array remains 828/842 and Zend/lang 4,211/5,599 byte-identically. `sprintf()`/`printf()` family value, width and precision positions now share PHP's missing/zero/limit/overflow validation and precedence without partial output. All feature, unsafe, Composer/Symfony S0-S3 and exact-final-binary performance gates pass; startup, retained `strlen()`, ordinary `sprintf()`/`printf()` and retained `sscanf()` paired medians are below +5%. The stale `${var}` interpolation deprecation shared by `strcasecmp.phpt`, `strcmp.phpt` and `strlen.phpt` is next; platform cryptography/locale groups, runtime-selected method/static array-reference context and the remaining strings/Zend gaps stay explicit work. |
