...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 4,079/5,599 with zero timeouts and zero crashes. `similar_text()` implements PHP's byte-oriented Oliver longest-common-substring partitioning, order-sensitive ties, percent output reference and PHP 8.5 weak/strict typed boundaries; a generic compiler correction also evaluates then rejects simple assignment results passed by reference. The exact delta is +1/-0 over `8395f772`, solely `bug78154.phpt`, with two byte-identical full manifests, every prior pass preserved and no other category movement; an adjacent upstream PHPT and a 14,641-pair PHP 8.5.9 differential sweep also pass. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; paired release changes are +0.826% for 100 empty requests and +3.042% for two million existing `levenshtein()` calls, below the +5% gate. Faster or Unicode-aware similarity, large/untrusted-input suitability, complex reference expressions, detached strict-call metadata and broader PHP 8.5 clusters remain explicit work. |
