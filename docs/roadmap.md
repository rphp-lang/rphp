...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 4,088/5,599 with zero timeouts and zero crashes. Unix/C-POSIX `escapeshellarg()`/`escapeshellcmd()` add byte-exact quoting, while synchronous `/bin/sh -c` `exec()`/`shell_exec()` implement binary stdout, last-line/output-array/status and no-output contracts; the supplying CLI flags and one-array-argument `implode()`/`join()` overload are included. The exact delta is +7/-0 over `80eaec31`, with every prior pass preserved and two byte-identical full manifests; five older failures advance only to independent output mismatches. Fourteen focused upstream PHPTs plus 510-byte and 74,898-case PHP 8.5.9 differential sweeps pass. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; paired release changes are +0.745% for 100 empty requests and +2.048% for two million existing `implode()` calls, below the +5% gate. Windows/non-C locales, untrusted-input safety, async/stream/timeout APIs, other process functions, real `-e` extended info, readline `-a`, internal Reflection arginfo and the later `.5`/`uniqid()`/`system()` blockers remain explicit work. |
