...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set is 4,110/5,599 (+1/-0) with zero timeouts or crashes. Nine include-path, extended file and CSV surfaces are now in the default build, including PHP 8.5 omitted-escape diagnostics. The 265-case focus moves from 53 to 85 passes (+32/-0), the full 897-case file audit from 104 to 135 (+31/-0), and the 158-case streams audit from 22 to 23 (+1/-0), without a lost pass. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. After rejecting +38.764% `file()` and +11.846% `fgetcsv()` candidates, the accepted 32-pair comparable lanes range from -3.306% to +0.942% paired, below the +5% gate. CLI-INI include-path restoration, network/custom wrappers, filters, sockets, resource lifetime, complete internal Reflection return-type metadata and broader compatibility remain explicit work. |
