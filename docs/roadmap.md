...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The accepted array-reference/l-value checkpoint raises the pinned PHP 8.2 corpus to 1,189/4,345 with no lost pass; next close the general return-by-reference call alias gap, then clear the six `SKIPIF` setup failures before the parse/compile clusters. |
