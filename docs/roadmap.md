...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 checkpoint is now 4,098/5,599 with zero timeouts and zero crashes. `array_merge_recursive()` and `array_replace_recursive()` supply the exercised key, scalar/object projection, copy-on-write, reference, recursion and next-index-overflow contracts while leaving their non-recursive counterparts unchanged. The exact delta is +1/-0 over `bfcc6bd1`, with every prior pass preserved and two byte-identical full manifests. Twenty-one of 28 focused upstream PHPTs pass; the remainder delimit independent parser, string-escape and array-intersection surfaces. All five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Paired startup, existing `array_replace()` and adjacent `array_merge()` medians change by +0.419%, -1.160% and -0.460%, below the +5% gate. The named blockers, complete array suite, adversarial recursion and broader compatibility remain explicit work. |
