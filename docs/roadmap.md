...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set reaches 4,194/5,599 with zero XFAIL, timeout or crash. The small user-sort checkpoint gives `usort()`, `uasort()` and `uksort()` PHP 8.5's stable two-to-five-element callback schedule and completes the ordinary-object `print_r()` renderer for inherited visibility-qualified/dynamic properties, nesting and recursion. The full array audit reaches 814/842 (+1/-0), while Zend/lang gains seven exact object-rendering passes (+7/-0). All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; accepted scalar-Long/impure paired medians are -1.951%/+0.424%, below the +5% gate. The remaining 1,109 Zend/lang failures, 14 array-suite failures and broader PHP compatibility remain explicit work. |
