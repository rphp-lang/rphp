...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,187/5,599 with zero XFAIL, timeout or crash. The recursive-array boolean-diagnostic checkpoint gives `array_merge_recursive()` and `array_replace_recursive()` value-sensitive `true given`/`false given` errors at fixed and variadic positions while preserving other type names, argument numbering, parameter naming and successful combiner behavior. The full array audit reaches 813/842, an exact +2/-0 delta, while Zend/lang has no status change. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; no runtime performance lane applies because only the rejected non-array cold path changed. The remaining 1,116 Zend/lang failures, 15 array-suite failures and broader PHP compatibility remain explicit work. |
