...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus remains 4,098/5,599 with zero timeouts and zero crashes and two byte-identical manifests. `sort()`, `rsort()`, `asort()`, `arsort()`, `ksort()`, `krsort()` and `array_multisort()` now share exercised PHP regular, numeric, string, C-locale string and natural comparison semantics, stable order, key reconstruction, live references, multi-column movement, diagnostics and exceptions. The adjacent 98-case sort cluster moves from 40 to 78 passes (+38/-0), and the complete 842-case array audit moves from 378 to 418 passes (+40/-0) with the same inherited seven timeouts and zero crashes. An original 2,671-result matrix including 1,103-entry inputs is byte-identical to PHP 8.5.9. All five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Paired startup, integer `sort()`, natural `ksort()` and `array_multisort()` medians change by +0.243%, -1.657%, -6.954% and +0.272%, below the +5% gate. Leading-dot literals, missing string escapes, five mixed-value comparison-schedule cases, locale-specific collation, inherited array-suite timeouts and broader compatibility remain explicit work. |
