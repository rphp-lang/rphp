...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,388/5,599 with zero timeouts and zero crashes. Successfully compiled inline and nested `eval()` source units now publish exact synthetic frames in live and stored traces, adding +1/-0 over `293709cc`. The five feature configurations, unsafe ratchet, Composer S0, Symfony S1/S2 and the CPU-pinned eval control pass. Multiline eval-argument line metadata, magic-call argument trace snapshots, exact Throwable-trace lifetime and the remaining PHP 8.5 failure clusters stay explicit compatibility work. |
