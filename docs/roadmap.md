...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 2,856/5,599 with zero timeouts and zero crashes. Deferred attribute constants and exact temporary-receiver lifetime add a combined 12 corpus-wide passes without losing one; ordinary CV method dispatch remains neutral, while the measured +8.8% ephemeral-receiver cost is retained as compatibility-first performance debt. Remaining attribute diagnostics, unknown INI directives, `disable_functions` and broader exception-unwind lifecycle stay explicit gaps. |
