...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 4,078/5,599 with zero timeouts and zero crashes. `md5()` implements the clean-room RFC 1321 one-shot digest, hexadecimal/raw output and PHP 8.5 weak/strict typed boundaries; valid `hash('md5', ...)` calls reuse it. The exact delta is +1/-0 over `0ccfebf1`, solely `gh19280.phpt`, with two byte-identical full manifests, every prior pass preserved and no other category movement; six adjacent unmodified MD5/hash PHPTs and the previously blocked `stristr.phpt` also pass. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; paired release changes are -0.778% for 100 empty requests and -1.466% for two million existing `hash('xxh128', ...)` calls, below the +5% gate. File/HMAC/streaming hashing, broader binary-string consumers, detached strict-call metadata and broader PHP 8.5 clusters remain explicit work. |
