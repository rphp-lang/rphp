...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 checkpoint closes all six `chunk_split()` cases with exact byte chunking/endings, empty/default/partial/extreme lengths, typed weak/strict boundaries, deterministic allocation failure and canonical direct fallback. Strings reaches 479/733 (+6/-0); array remains 828/842 and Zend/lang 4,206/5,599 with no lost pass or other outcome movement. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Exact-final-binary CPU-pinned medians range from -30.602% to +2.218% across the three affected paths, adjacent `str_split()` and startup controls, below +5%. The eight visible `str_replace()`/`str_ireplace()` array/reference/count/type cases are next; wider string-offset writes, typed-parameter byte provenance, detached-callback alias spelling, 32-bit execution and remaining strings/Zend gaps remain explicit work. |
