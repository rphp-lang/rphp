...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus remains byte-identical at 4,109/5,599 (+0/-0) with zero timeouts or crashes. `count()`/`sizeof()` and `min()`/`max()` now cover exercised signatures, constants, typed/weak errors, Countable, iterative recursive counting, array/variadic extrema, references, exact numeric strings, arrays/objects, recursive comparisons, ties and NaN. Their 23-case focused cluster moves from 1 to 22 passes (+21/-0), and the complete 842-case array audit moves from 564 to 585 (+21/-0), both without a lost pass; the sole focused blocker is independent missing `opendir()`. Five original E2E tests, all five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Paired startup/count/extrema/comparison medians range from -57.694% to +1.892%, below the +5% gate. Complete reflected internal metadata, the remaining array suite and broader compatibility remain explicit work. |
