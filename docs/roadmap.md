...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,390/5,599 with zero timeouts and zero crashes. Declared function arguments are read live after assignment or unset while true extra arguments retain stable storage; method eval scopes and active eval traces now share those live values, adding +2/-0 over `87006c82`. The five feature configurations, unsafe ratchet, Composer S0, Symfony S1/S2 and the CPU-pinned method-eval control pass. Multiline eval-argument line metadata, exact Throwable-trace lifetime and the remaining PHP 8.5 failure clusters stay explicit compatibility work. |
