...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set is 4,140/5,599 (+23/-0) with zero timeouts or crashes. A callback-dispatch batch now covers named/variadic `call_user_func()`, reference-aware `call_user_func_array()`, both `forward_static_call*()` forms, three-argument `is_callable()`, explicit Closure forwarding and both `ReflectionClass` construction forms. Its 36-case focus moves from 16 to 33 passes (+17/-0), while PHP 8.5.9 passes all 36. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; seven 32-pair AMD64 controls stay between -0.600% and +4.562% paired, below the +5% gate. The remaining focus is one named-variadic `call_user_func_array()` runtime frame and two independent parser forms; complete Reflection metadata and broader compatibility remain explicit work. |
