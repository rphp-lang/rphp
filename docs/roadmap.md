...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus remains 4,098/5,599 with zero timeouts and zero crashes. Six associative set functions now implement exact-key/PHP-string value comparison or user-key callbacks with exercised validation order, small-array comparator scheduling, snapshots, live references, short-circuiting and insertion-order output. All 42 runtime-reachable cases in the supplying 55-case `ext/standard` cluster pass in debug and release; 13 stop at independent parser gaps, while PHP 8.5.9 passes 55/55. The exact Zend/lang delta is intentionally +0/-0 with two byte-identical full manifests, and a 300-result clean-room matrix is byte-identical to PHP. All five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Paired startup and existing `array_diff_key()` medians change by +0.094% and -0.830%, below the +5% gate. Large-array comparator traces, the parser blockers, `array_change_key_case()`, the six value-callback set functions, the complete array suite and broader compatibility remain explicit work. |
