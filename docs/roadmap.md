...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,202/5,599 with zero XFAIL, timeout or crash. The latest checkpoint closes `htmlentities23.phpt` and `bug49785.phpt` with distinct EUC-JP/Big5 unit validation plus PHP-compatible reject, substitute and ignore recovery. Strings reaches 436/733 (+2/-0), with 212 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Six exact-binary CPU-pinned paired medians range from -1.459% to +2.621%, below the +5% gate. The 12-case slash-escaping cluster is next; unknown entity-encoding warnings, 32-bit execution, remaining strings/Zend gaps and wider PHP compatibility remain explicit work. |
