...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set reaches 4,187/5,599 with zero XFAIL, timeout or crash. The array-map callback-contract checkpoint validates callbacks before array arguments, aligns callable-member and variadic diagnostics, resolves but does not invoke empty magic callbacks, and preserves aliases, references and recursive topology through the null/COW path. The full array audit reaches 801/842, an exact +4/-0 delta; Zend/lang gains two related cases with no loss. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; every same-work release lane remains below the +5% median ceiling. The remaining 1,116 Zend/lang failures, 27 array-suite failures and broader PHP compatibility remain explicit work. |
