...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,202/5,599 with zero XFAIL, timeout or crash. The latest checkpoint closes all three ordinary `ENT_DISALLOWED` cases with document-specific literal and numeric-reference validity, reversible single-byte encoding maps, unrepresentable replacement handling and validated SJIS units. Strings reaches 427/733 (+3/-0), with 221 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Exact-binary CPU-pinned paired medians remain below the +5% gate for ASCII encoding (-2.570%), `double_encode=false` (-4.111%), default decoding (+2.101%), ordinary concatenation (+0.572%) and startup (-0.244%). The three-case invalid-UTF-8 substitution/ignore family is next; EUC-JP, 32-bit execution, remaining strings/Zend gaps and wider PHP compatibility remain explicit work. |
