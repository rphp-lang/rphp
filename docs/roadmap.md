...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 2,975/5,599 (56.164%) with zero timeouts and zero crashes. The internal `Random\IntervalBoundary` enum, enum-parent rejection and symbolic enum trace arguments add an exact +2/-0; the complete `Deprecated` directory is 47/47 and the standard exception-handler directory remains 10/10. The missing `NoDiscard` built-in attribute, remaining INI directives and `disable_functions` are explicit next gaps; `Randomizer::getFloat()` is not yet admitted. The ordinary file-request median control remains below the five-percent performance ceiling, with its noisy paired upper tail disclosed in the detailed evidence. |
