...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 malformed-`unserialize()`/reference checkpoint reaches 628/733 strings and 4,214/5,599 Zend/lang, an exact combined +7/-0, while array remains 828/842. General malformed offsets, legacy `C:` payload order, uppercase `R:` identity and reference-backed Throwable state now hold. All feature, unsafe, Composer/Symfony S0-S3 and exact-final-binary performance gates pass; ordinary no-`R:` decoding remains below +5%. Monitored supported debt is 1,110. The rejected platform `crypt()` approach remains deferred; portable `parse_url()` authority/path ambiguity is next. |
