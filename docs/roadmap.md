...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 Zend/lang pass set remains 4,202/5,599 with no timeout or crash. The latest checkpoint closes all 11 supplying `trim()`/`ltrim()`/`rtrim()`/`chop()` cases with PHP-byte default masks, complete charlist ranges and diagnostics, weak/strict boundaries and public alias metadata. Strings reaches 461/733 (+11/-0), with 187 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Exact-binary CPU-pinned paired medians range from -7.014% to +4.703% across the two trim paths and string/callback/startup controls, below the +5% gate. The six-case `str_increment()`/`str_decrement()` cluster is next; typed-parameter byte provenance, detached-callback alias spelling, 32-bit execution and remaining strings/Zend gaps remain explicit work. |
