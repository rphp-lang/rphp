...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 2,878/5,599 with zero timeouts and zero crashes. Startup `error_reporting` admits 32 previously unsupported cases: 14 pass and 18 now expose their independent gaps, with no lost pass; the wider attempted denominator honestly lowers the headline rate to 54.508%. The localized +8.8% ephemeral-receiver cost from the earlier lifetime checkpoint remains compatibility-first performance debt. Remaining INI directives, `disable_functions` and broader exception-unwind lifecycle stay explicit gaps. |
