...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set is 4,148/5,599, +8/-0 from the preceding checkpoint with zero timeouts or crashes. The runtime-introspection batch adds ordered Core/user constant inventory and collision protection, `zend_version()`, explicit-collector `gc_status()` state/timing, legacy `ReflectionParameter` construction/type APIs, late-static `ReflectionMethod::createFromMethodName()`, trait aliases and closure method rendering. Its 18-case focus moves from 0 to 13 passes while PHP 8.5.9 passes all 18. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; four 32-pair AMD64 controls stay between -0.442% and +3.041% paired, below the +5% gate. Automatic threshold GC, complete Reflection metadata/debug projection and five named focus failures remain explicit work. |
