...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 `base64_decode()` checkpoint closes all five strict/invalid-byte failures while retaining the whitespace pass, with one typed PHP-byte handler for loose/strict decoding, padding, conversion order, calls, Reflection and binary COW. Strings reaches 554/733 (+5/-0); array remains 828/842 and Zend/lang remains 4,209/5,599, with no lost pass, new timeout or crash. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Exact-final-binary CPU-pinned decode, encode and startup controls range from -8.617% to +3.105%, below +5%. The five quoted-printable failures are next; typed encoder boundaries, streaming codecs, platform cryptography and remaining strings/Zend gaps stay explicit work. |
