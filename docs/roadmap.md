...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,196/5,599 with zero XFAIL, timeout or crash. The latest checkpoint adds flag-aware, single-pass and byte-exact `htmlspecialchars_decode()`, AMD64 `decbin()`, exact `chr()` byte output and binary-aware `ord()` while preserving weak scalar calls. Its 11 supplying cases match PHP 8.5.9 at 10 passes plus one 32-bit skip; strings reaches 305/733 (+9/-0), while array stays byte-identical at 822/842. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; two CPU-pinned 32-pair runs keep every relevant startup/string/entity/decoder/`ord()` median below the +5% gate and publish candidate-only `decbin()` throughput without an unavailable-baseline ratio. Source-literal binary provenance, remaining `ord()` deprecations, other base conversion, full named entities, malformed-UTF-8 replacement, the remaining 1,107 Zend/lang failures, six array-suite failures and broader PHP compatibility remain explicit work. |
