...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 checkpoint closes all six `str_increment()`/`str_decrement()` cases with exact alphanumeric carry/borrow, growth/underflow, byte provenance, validation, weak/strict calls and documented-polyfill primitives. Strings reaches 467/733 (+6/-0) and Zend/lang 4,206/5,599 (+4/-0 through general in-bounds string-offset writes); array remains 828/842 and no prior pass is lost. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Exact-final-binary CPU-pinned medians range from -1.240% to +1.608% across the affected assignment, array, string and startup controls, below +5%. The six-case `str_split()` byte-chunking/validation cluster is next; wider string-offset writes, typed-parameter byte provenance, detached-callback alias spelling, 32-bit execution and remaining strings/Zend gaps remain explicit work. |
