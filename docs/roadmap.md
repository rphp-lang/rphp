...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains exactly 4,171/5,599, +0/-0 from the preceding checkpoint with zero XFAIL, timeout or crash. The `var_export()` checkpoint adds exact nested array/object formatting, binary-safe quoting, property projection and recursive-value warnings. The full array audit reaches 780/842, +6/-0, with no timeout or crash; the measured 43-case `var_export` cluster gains 18 exact passes and removes its recursive crash. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; both exact release workloads improve materially against the retained baseline. The remaining 1,132 Zend/lang failures, 48 array-suite failures and broader PHP compatibility remain explicit work. |
