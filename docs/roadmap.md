...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 `parse_url()` authority checkpoint reaches 629/733 strings, exactly +1/-0, while array remains 828/842 and Zend/lang remains 4,214/5,599. All 14 executable upstream `parse_url` cases pass, up from two; schemeless ports, opaque paths, empty ports, IPv6 and `file:///` drive paths now share a general parser contract. All feature, unsafe, Composer/Symfony S0-S3 and exact-final-binary performance gates pass. Monitored supported debt is 1,109. The 14-case platform `crypt()` family remains deferred; portable `get_meta_tags()` is next. |
