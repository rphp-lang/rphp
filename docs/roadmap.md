...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 4,075/5,599 with zero timeouts and zero crashes. `array_fill_keys()` implements PHP 8.5 key conversion, duplicate order, reference detachment, mixed-value identity/COW and diagnostic ordering. The exact delta is +1/-0 over `cb107ee1`, solely `bug45877.phpt`, with two byte-identical full manifests, every prior pass preserved and no other category movement; the five adjacent unmodified standard-library PHPTs also pass. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; paired release changes are +0.382% for 100 empty requests and -3.857% for 500,000 existing `array_fill()` calls, below the +5% gate. Huge-allocation failures, the complete standard array suite and broader PHP 8.5 clusters remain explicit work. |
