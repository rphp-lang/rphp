...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus moves from 4,108 to 4,109/5,599 (+1/-0) with zero timeouts or crashes and two byte-identical manifests. `in_array()`, `array_search()`, `array_key_exists()`/`key_exists()`, `array_sum()` and `array_product()` now cover exercised signatures, conversions, canonical keys, loose/strict recursive lookup, references/COW, reentrant warnings, numeric-kind transitions and overflow; exact large numeric-string comparison and CV/CV operator diagnostics also match PHP 8.5. Their 42-case focused cluster moves from 15 to 31 passes (+16/-0), and the complete 842-case array audit moves from 546 to 564 (+18/-0), both without a lost pass. Five original E2E tests, all five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Comparable paired startup/lookup/key/aggregate/control medians range from -51.410% to +0.313%, below the +5% gate; the now-correct mixed numeric-kind lane is disclosed separately for performance follow-up. Three focused parser gaps, two missing-`opendir()` gaps, complete reflected internal metadata and broader compatibility remain explicit work. |
