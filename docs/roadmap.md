...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 2,883/5,599 with zero timeouts and zero crashes. Exception presentation INI settings add five exact passes without losing one, and the headline rate recovers to 54.571% after the preceding honest denominator expansion. The localized +8.8% ephemeral-receiver cost from the earlier lifetime checkpoint remains compatibility-first performance debt. Remaining INI directives, `disable_functions` and broader exception-unwind lifecycle stay explicit gaps. |
