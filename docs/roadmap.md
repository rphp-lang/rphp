...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus moves from 4,107 to 4,108/5,599 (+1/-0) with zero timeouts or crashes and two byte-identical manifests. `array_keys()`, `array_values()`, `array_flip()`, `array_count_values()` and `array_rand()` now cover exercised signatures, filtering, canonical keys, warnings, references/COW, uniform ordered subsets and error precedence; null/string loose comparison also matches PHP 8.5. Their 33-case focused cluster moves from 10 to 26 passes (+16/-0), and the complete 842-case array audit moves from 525 to 546 (+21/-0), both without a lost pass. Four original E2E tests, all five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Balanced startup/projection/count-flip/random/comparison/control medians range from -42.381% to +3.572%, below the +5% gate. Six focused parser, missing-`opendir()` and case-insensitive-literal gaps, complete reflected internal metadata, near-limit allocation and broader compatibility remain explicit work. |
