...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 2,973/5,599 (56.126%) with zero timeouts and zero crashes. PHP 8.5 uncaught-exception handler dispatch and callable-to-Closure deprecation metadata are now covered; the exact adjacent delta is +18/-0, the complete `Deprecated` directory is 46/47 and the standard exception-handler directory is 10/10. The Random enum type-validation dependency, remaining INI directives and `disable_functions` are explicit next gaps. The ordinary file-request control remains below the five-percent performance ceiling. |
