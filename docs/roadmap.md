...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 2,902/5,599 with zero timeouts and zero crashes. Request-local `precision` and corrected float presentation add 19 exact passes without losing one, while all 18 precision-only cases are now honestly attempted; the headline rate is 54.786%. Common exact float-to-string conversion remains below the five-percent performance ceiling. The localized +8.8% ephemeral-receiver cost from the earlier lifetime checkpoint remains compatibility-first performance debt. Remaining INI directives, `disable_functions` and broader exception-unwind lifecycle stay explicit gaps. |
