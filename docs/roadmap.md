...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is 2,446/5,599 with zero crashes or timeouts. The focused lazy-object frontier is 206/223: Reflection stringification preserves lazy state and property metadata/order, while final proxies, destructor trees and formatted `%s` conversion retain their admitted contracts. Internal classes, weak references, cycle collection and broader shutdown ordering remain next. |
