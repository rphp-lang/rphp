...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 scalar-string checkpoint reaches 620/733 strings, exactly +3/-0 from `strcasecmp.phpt`, `strcmp.phpt` and `strlen.phpt`, while array remains 828/842 and Zend/lang 4,211/5,599 byte-identically. `strlen()` now honors request precision and comparisons plus nested `print_r()` preserve PHP bytes; PHP 8.5.9's `${var}` deprecation remains intact. All feature, unsafe, Composer/Symfony S0-S3 and exact-final-binary performance gates pass, with all nine lanes below +5%. The four-case `setlocale()` scalar/long-name boundary is next; the 14-case platform `crypt()` family and remaining string/Zend gaps stay explicit work. |
