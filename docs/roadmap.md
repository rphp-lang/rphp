...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,438/5,599 with zero timeouts and zero crashes. No-argument `get_parent_class()` emits PHP 8.5's handler-aware deprecation before lexical lookup or the global false return; throwing handlers interrupt both outcomes and explicit arguments remain unchanged. The exact delta is +2/-0 over `a82bfeb5`, with two byte-identical full manifests and no other category movement. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; no hot-path performance gate applies to the isolated deprecated branch. Remaining trait, compile, CLI-capability and other PHP 8.5 clusters stay explicit compatibility work. |
