...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,441/5,599 with zero timeouts and zero crashes. Trait method and closure `__CLASS__` reads bind to the exact final composition selected by dispatch, independently of late-static scope; inherited, reused, nested, aliased, private and reentrant parent paths retain PHP 8.5 semantics. The exact delta is +3/-0 over `462a7bd0`, with two byte-identical full manifests and no other category movement. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; the affected five-million-call path measures +4.553% independently and +4.646% paired, below the +5% gate. Trait property defaults, deprecated parent callables, missing library functions and other PHP 8.5 clusters stay explicit work. |
