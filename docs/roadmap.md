...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains exactly 4,185/5,599 with zero XFAIL, timeout or crash. The array-walk live-mutation checkpoint replaces snapshot traversal with a reference/COW-safe live cursor across unset, append, replacement, recursion, reentrancy and exception traces; all 45 `array_walk` cases pass. The full array audit reaches 797/842, an exact +7/-0 delta. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; startup and by-value controls remain below the +5% median ceiling while the admitted by-reference lanes improve materially. The remaining 1,118 Zend/lang failures, 31 array-suite failures and broader PHP compatibility remain explicit work. |
