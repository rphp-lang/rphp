...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus moves from 4,100 to 4,103/5,599 (+3/-0) with zero timeouts or crashes and two byte-identical manifests. `range()`, `array_fill()`, `array_combine()` and `array_merge()` now cover exercised construction modes, conversions, key/reference/COW behavior, reentrant diagnostics and safe size limits; ordinary append reports next-key exhaustion. Their 52-case focused cluster moves from 14 to 43 passes (+29/-0) and removes six timeouts; the complete 842-case array audit moves from 463 to 496 passes (+33/-0), removes all six inherited timeouts and retains zero crashes. Four original E2E tests, the clean-room oracle, all five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Comparable paired startup/construction/append medians range from -21.252% to +4.349%, below the +5% gate. Seven focused parser/output/dump/prelude gaps, successful near-limit allocation, complete reflected internal metadata, exhaustive multibyte range behavior, by-reference append overflow and broader compatibility remain explicit work. |
