...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 `setlocale()` checkpoint reaches 624/733 strings, exactly +4/-0 from the strict/weak/null/long-name cluster, while array remains 828/842 and Zend/lang 4,211/5,599 byte-identically. PHP's top-level-versus-array conversion order, handler diagnostics and Reflection shape now hold inside the portable `C`/`POSIX` subset. All feature, unsafe, Composer/Symfony S0-S3 and exact-final-binary performance gates pass, with startup, retained `strlen()` and direct/dynamic locale calls below +5%. The 14-case platform `crypt()` family is next; wider host-locale and remaining string/Zend gaps stay explicit work. |
