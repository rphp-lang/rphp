...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 2,864/5,599 with zero timeouts and zero crashes. Logical attribute-constructor traces and canonical `ReflectionAttribute::newInstance` diagnostics add eight exact passes without losing one; sparse sidecars leave ordinary frame layout unchanged and measured callback controls stay within their five-percent ceiling. The localized +8.8% ephemeral-receiver cost from the preceding lifetime checkpoint remains compatibility-first performance debt. Unknown INI directives, `disable_functions` and broader exception-unwind lifecycle stay explicit gaps. |
