...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains at 4,109/5,599 (+0/-0) with zero timeouts or crashes. Eight stream registry/content/copy/line/truncation functions are now in the default build with corrected `stream_is_local($stream)` metadata. The 93-case focus moves from 0 to 17 passes (+17/-0), the full 158-case streams audit from 17 to 22 (+5/-0), and the 897-case file audit from 92 to 104 (+12/-0), without a lost pass; array and non-Windows directory controls are byte-identical. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. CPU-pinned 32-pair comparable lanes range from -0.927% to +1.088% paired, below the +5% gate. Network wrappers/transports, filters, custom wrappers, sockets/nonblocking I/O, complete internal Reflection return-type metadata and broader compatibility remain explicit work. |
