...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set is 4,202/5,599 with zero XFAIL, timeout or crash. The latest checkpoint closes all 79 attempted cases in the 101-case `printf*`/`sprintf*`/`vprintf*`/`vsprintf*` family through one PHP-byte engine for positions, stars, flags, widths, precisions, conversions, binary output, arity and diagnostics. Strings reaches 406/733 (+63/-0), with 242 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Two CPU-pinned 32-pair controls keep 500,000 ordinary `sprintf()` calls at +3.456%/+4.005%, below the +5% gate, while startup improves by more than 4%. Locale-specific formatting, 32-bit execution, remaining strings/Zend gaps and broader PHP compatibility remain explicit work. |
