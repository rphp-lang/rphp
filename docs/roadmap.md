...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,202/5,599 with zero XFAIL, timeout or crash. The latest checkpoint closes the five ordinary HTML4/HTML5 named-entity encode/decode cases with canonical and alias tables, multi-codepoint values, document/quote modes, exact unknown-name rejection and byte-compatible UTF-8 table keys. Strings reaches 424/733 (+6/-0), with 224 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Exact-binary CPU-pinned paired medians remain below the +5% gate for ASCII encoding (+1.611%), default decoding (-9.126%), ordinary concatenation (+2.138%) and startup (+0.464%). The three-case `ENT_DISALLOWED` family is the next coherent entity cluster; 32-bit execution, remaining strings/Zend gaps and wider PHP compatibility remain explicit work. |
