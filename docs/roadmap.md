...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set is 4,201/5,599 with zero XFAIL, timeout or crash. The latest checkpoint closes all six ordinary failures in the complete recursive array selection through shared property-order, unsigned-format, exponent-precedence, callback-trace, `ArrayObject::append()` and deprecated object-cursor contracts: array reaches 828/842 (+6/-0) with zero fail, 13 skip and one unsupported, while strings reaches 309/733 (+4/-0). All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; `PhpObject` remains 72 bytes and two CPU-pinned 32-pair ordinary-`array_map()` controls keep paired medians at +4.286% and +3.146%, below the +5% gate. Complete formatting and `ArrayObject`, deprecated cursor mutation/reentrancy edges, 32-bit execution, the inherited strings timeout, 339 strings failures, 1,102 Zend/lang failures and broader PHP compatibility remain explicit work. |
