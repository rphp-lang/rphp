...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set reaches 4,196/5,599 with zero XFAIL, timeout or crash. The `debug_zval_dump()` checkpoint derives interned/refcount annotations from PHP-visible roots and immutable-literal provenance while preserving the 16-byte `Value` layout and public `var_dump()`/`print_r()` output. The full array audit reaches 816/842 (+2/-0), while Zend/lang gains two exact enum-refcount passes (+2/-0). All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; two accepted paired-median runs keep ordinary append at +1.301%/+1.024% and literal COW at +2.813%/+1.736%, below the +5% gate. The remaining 1,107 Zend/lang failures, 12 array-suite failures and broader PHP compatibility remain explicit work. |
