...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains exactly 4,171/5,599, +0/-0 from the preceding checkpoint with zero XFAIL, timeout or crash. The array-set-operations checkpoint gives `array_diff()` and `array_intersect()` their PHP 8.5 variadic, diagnostic, conversion, key/order and live-reference contract. The full array audit reaches 774/842, +41/-0, with no timeout or crash. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; the relevant two-array release lanes remain below the +5% median gate. The remaining 1,132 Zend/lang failures, 54 array-suite failures and broader PHP compatibility remain explicit work. |
