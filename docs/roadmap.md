...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains at 4,109/5,599 (+0/-0) with zero timeouts or crashes. `chdir()`, `opendir()`, `readdir()`, `rewinddir()`, `closedir()` and `scandir()` now share request-owned directory streams, PHP diagnostics, implicit-handle deprecations, scan ordering and request CWD restoration. The 39-case focus moves from 0 to 30 passes (+30/-0), the 49-case non-Windows directory audit from 0 to 27 (+27/-0), and the 842-case array audit from 585 to 590 (+5/-0), all without a lost pass. Three original regressions, all five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Paired startup/unchanged-filesystem medians range from -1.751% to +1.542%, below the +5% gate. The `dir()` object, remaining context/parser/SKIPIF gaps, complete reflected internal metadata and broader compatibility remain explicit work. |
