...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,202/5,599 with zero XFAIL, timeout or crash. The latest checkpoint closes all three invalid-UTF-8 substitution/ignore cases with maximal-subpart recovery, binary source-literal provenance, ignore precedence and the general Windows-1252 named-entity path. Strings reaches 434/733 (+7/-0), with 214 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. One still-failing case advances visibly to its EUC-JP/Big5 output boundary. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Exact-binary CPU-pinned paired medians remain below the +5% gate for byte-literal lexing (-1.013%), ASCII encoding (+2.041%), `double_encode=false` (+0.196%), default decoding (-2.226%), ordinary concatenation (+1.419%) and startup (-0.143%). EUC-JP/Big5 validation is next; 32-bit execution, remaining strings/Zend gaps and wider PHP compatibility remain explicit work. |
