...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,425/5,599 with zero timeouts and zero crashes. Eval and included-source valid-syntax compiler failures now remain uncatchable compile fatals, while catchable syntax errors and `@eval` reporting/restoration retain their distinct PHP boundaries; the exact delta is +2/-0 over `0de2e3c4`, with three remaining cases advanced to known later diagnostics. All five feature configurations, unsafe ratchet, Composer S0, Symfony S1/S2/S3 gates and the CPU-pinned successful-eval control pass. Internal-class alias output, the remaining eval diagnostics and other PHP 8.5 clusters stay explicit compatibility work. |
