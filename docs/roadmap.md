...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set reaches 4,185/5,599, +1/-0 from the preceding checkpoint with zero XFAIL, timeout or crash. The extract-reentrancy checkpoint snapshots destructive/reentrant targets before scope writes, preserves `EXTR_REFS`, COW and explicit `$GLOBALS` cells, applies every `$this` mode, and keeps inactive conditional `global` declarations local. The full array audit reaches 790/842, +3/-0, and the focused extract cluster passes 18/18. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; all four affected release lanes remain below the +5% median ceiling. The remaining 1,118 Zend/lang failures, 38 array-suite failures and broader PHP compatibility remain explicit work. |
