...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set is 4,201/5,599 with zero XFAIL, timeout or crash. The latest checkpoint closes the complete 38-case `stripos()`/`strrpos()`/`strripos()` cluster through shared PHP-byte offsets, negative boundaries, typed conversion, binary provenance and ASCII-only folding: strings reaches 343/733 (+34/-0), with 305 failures, 54 skips, 30 unsupported cases and one retained timeout; array remains 828/842 with zero ordinary failure. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. Two CPU-pinned 32-pair controls keep ordinary `strrpos()` at +3.741%/+3.697%, below the +5% gate, while folded search improves by 48%. The independent `strpos()` family, 32-bit execution, remaining strings/Zend gaps and broader PHP compatibility remain explicit work. |
