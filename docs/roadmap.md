...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus remains 4,098/5,599 with zero timeouts and zero crashes. Six user-value set functions now cover value, exact-key/value and user-key/value comparison with exercised PHP callback scheduling, validation, duplicates, snapshots, live references and insertion-order output. `array_diff_key()` and `array_intersect_key()` also have their one-required-array variadic PHP 8.5 contracts. The supplying 33-case `ext/standard` cluster moves from 0 to 17 passes in debug and release; 16 independent parser, formatting or later ordinary-`array_intersect()` blockers remain, while PHP 8.5.9 passes 33/33. The exact Zend/lang delta is intentionally +0/-0 with two byte-identical full manifests, and a 300-result clean-room matrix is byte-identical to PHP. All five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Paired startup, 500,000-entry `array_diff_key()` and unchanged 500,000-call `array_intersect()` medians change by +0.137%, +0.072% and -2.046%, below the +5% gate. An ordinary variadic `array_intersect()` candidate was reverted after remaining above the gate. Arbitrary-large comparator traces, `array_change_key_case()`, ordinary variadic `array_intersect()`, the complete array suite and broader compatibility remain explicit work. |
