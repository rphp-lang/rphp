...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus remains 4,098/5,599 with zero timeouts and zero crashes and two byte-identical manifests. `array_change_key_case()`, `key_exists()`, `strnatcasecmp()`, `natsort()` and `natcasesort()` now cover exercised PHP signatures, case/collision rules, natural ordering, by-reference mutation, stable mixed-key order, aliases, object conversion and exceptions; `strnatcmp()` shares the corrected initial-zero and whitespace semantics. The adjacent 39-case `ext/standard` cluster moves from 6 to 30 passes (+24/-0), and the complete 842-case array audit moves from 355 to 378 passes (+23/-0) with the same inherited seven timeouts and zero crashes. A clean-room matrix of 1,444 string pairs, 80 sorts and 160 key transforms is byte-identical to PHP 8.5.9. All five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Paired startup, 500,000-call `strnatcmp()` and 100,000-call natural `asort()` medians change by +0.222%, -2.638% and +2.629%, below the +5% gate. Remaining `array_key_exists()` edges, string escapes, the leading-dot numeric parser form, ordinary sort behavior, arbitrary-large comparator traces, inherited array-suite timeouts and broader compatibility remain explicit work. |
