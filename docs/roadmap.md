...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,187/5,599 with zero XFAIL, timeout or crash. The array-multisort argument-contract checkpoint gives fixed and variadic arguments one stateful flag/column classifier with exact TypeError/ValueError diagnostics, and confines PHP's stable small-input comparison schedule to reentrant two-to-five-row `array_multisort()` calls while retaining the preceding large-input merge path. The full array audit reaches 811/842, an exact +4/-0 delta, while Zend/lang has no status change. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; startup and 150,000-row multi-column release lanes remain below the +5% median ceiling. The five non-transitive mixed-sort schedule cases, throwing-comparison partial permutation, remaining 1,116 Zend/lang failures, 17 array-suite failures and broader PHP compatibility remain explicit work. |
