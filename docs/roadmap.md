...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains exactly 4,171/5,599, +0/-0 from the preceding checkpoint with zero XFAIL, timeout or crash. The leading-dot-float checkpoint admits `.5`, signs, exponents and digit separators while preserving concatenation and longer dot-token boundaries. The full array audit reaches 717/842, +24/-0; all 58 affected cases advance beyond parsing, with 34 exposing independent output differences and no timeout or crash. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. No runtime performance gate applies because the change is lexer-only. The remaining 1,132 Zend/lang failures, 111 array-suite failures and broader front-end and PHP compatibility remain explicit work. |
