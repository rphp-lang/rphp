...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main corpus moves from 4,098 to 4,100/5,599 (+2/-0) with zero timeouts or crashes and two byte-identical manifests. `array_chunk()`, `array_slice()`, `array_reverse()` and `array_pad()` now share exercised key projection, boundary, padding, copy-on-write, reference, conversion, diagnostic, exception and signature semantics. Their 55-case focused cluster moves from 6 to 50 passes (+44/-0) and loses its timeout; the complete 842-case array audit moves from 418 to 463 passes (+45/-0), reduces inherited timeouts from seven to six and retains zero crashes. Four original E2E tests and all five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass. Paired startup, `array_chunk()`, `array_slice()`, `array_reverse()` and `array_pad()` medians change by +0.308%, -1.797%, -0.087%, -66.441% and +1.506%, below the +5% gate. Five focused parser/output/resource gaps, the six inherited array-suite timeouts, complete reflected internal metadata, mutating `array_splice()` and broader compatibility remain explicit work. |
