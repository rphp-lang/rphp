...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 `get_meta_tags()` checkpoint reaches 630/733 strings, exactly +1/-0, while array remains 828/842 and Zend/lang remains 4,214/5,599. Typed file/include-path behavior and a clean-room metadata byte parser now cover ordinary and malformed tag boundaries; a 39-case transcript is byte-identical to PHP 8.5.9. All feature, unsafe, Composer/Symfony S0-S3 and exact-final-binary performance gates pass. Monitored supported debt is 1,108. The 14-case platform `crypt()` family remains deferred; portable `hebrev()` is next. |
