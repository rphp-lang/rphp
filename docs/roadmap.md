...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,002/5,599 (56.674%) with zero timeouts and zero crashes. The `NoDiscard` ABI, callable diagnostics and six pinned `(void)` cases add an exact +27/-0; the NoDiscard directory is 20/25. Its remaining cases depend on `DateTimeImmutable`, `zend_test` or skipped native extensions, while `ReflectionProperty::getHook()`, remaining INI directives, `disable_functions` and `Randomizer::getFloat()` remain explicit gaps. Both ordinary file-request and call-path median controls remain below the five-percent performance ceiling. |
