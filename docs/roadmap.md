...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 4,081/5,599 with zero timeouts and zero crashes. `strtok()` implements PHP's request-local byte cursor and warning transitions, `str_shuffle()` adds unbiased byte permutation, and existing by-reference `shuffle()` shares the lazy request RNG while reindexing and reporting the exact array TypeError. The exact delta is +2/-0 over `9b56808a`, solely `bug76047.phpt` and `gh13145.phpt`, with two byte-identical full manifests, every prior pass preserved and no other category movement; 17 focused upstream PHPTs and a 7,161-case PHP 8.5.9 tokenizer sweep also pass. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; paired release changes are -0.424% for 100 empty requests and -3.446% for 500,000 existing `shuffle()` calls, below the +5% gate. PHP-exact seeded randomness, other random APIs, the later `sprintf()`/`array_diff_assoc()` blockers, reentrant tokenizer mutation and broader PHP 8.5 clusters remain explicit work. |
