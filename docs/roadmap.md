...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,196/5,599 with zero XFAIL, timeout or crash. The numeric legacy-entity checkpoint adds byte-exact `html_entity_decode()` output for ISO-8859-1/15/5, CP866, KOI8-R, MacRoman and Windows-1251/1252, including aliases, quote/document flags, invalid forms and warning fallback. The strings audit reaches 296/733 (+10/-0 from the preceding checkpoint), while array stays byte-identical at 822/842. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; two CPU-pinned paired-median runs keep startup, unchanged string, UTF-8 decode and `htmlspecialchars()` controls below the +5% gate, while candidate-only legacy decode throughput is published without a baseline ratio. Complete named-entity tables, multi-code-point entities, `ENT_DISALLOWED`, malformed-UTF-8 replacement, optional encoding extensions, the remaining 1,107 Zend/lang failures, six array-suite failures and broader PHP compatibility remain explicit work. |
