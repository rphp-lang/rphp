...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,395/5,599 with zero timeouts and zero crashes. Shared trait bytecode keeps lexical trait scope while cold trace/type-diagnostic paths expose the concrete composing class and selected alias, adding +5/-0 over `e2d17bad`. The five feature configurations, unsafe ratchet, Composer S0, all Symfony S1/S2/S3 gates and CPU-pinned cold-link/hot-call controls pass. The missing magic-property entry frame in `bug69180-backtrace.phpt`, multiline eval-argument line metadata, exact Throwable-trace lifetime and the remaining PHP 8.5 clusters stay explicit compatibility work. |
