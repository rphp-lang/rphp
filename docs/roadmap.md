...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,396/5,599 with zero timeouts and zero crashes. Detached magic-property methods keep their return boundary while live traces cross an exact current-instruction caller link, making `bug69180-backtrace.phpt` exact for +1/-0 over `3a2a2ae7`. The five feature configurations, unsafe ratchet, Composer S0, all Symfony S1/S2/S3 gates and CPU-pinned repeated-`__get` control pass. Multiline eval-argument line metadata, exact Throwable-trace lifetime and the remaining PHP 8.5 clusters stay explicit compatibility work. |
