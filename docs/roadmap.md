...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set is 4,111/5,599 (+1/-0) with zero timeouts or crashes. `fprintf()`, `vfprintf()`, `sscanf()` and `fscanf()` are now in the default build behind the selectable `formatted-io` feature. Their 105-case focus moves from 0 to 84 passes (+84/-0), and the full 897-case file audit from 135 to 181 (+46/-0), without a lost pass. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass. The relevant 32-pair startup and unchanged-`sprintf()` controls range from -0.142% to +1.691% paired, below the +5% gate; new scanf/formatted-stream paths remain 3.8–5.1× slower than PHP and are visible optimization opportunities. Independent process, stream-warning, object-conversion, parser/reference, older `printf()`, Reflection and broader compatibility gaps remain explicit work. |
