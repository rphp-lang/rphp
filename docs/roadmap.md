...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 main pass set remains 4,187/5,599 with zero XFAIL, timeout or crash. The user-sort callback-contract checkpoint gives `usort()`, `uasort()` and `uksort()` shared temporary-value and hard-reference diagnostics, preserves reference cells through the final permutation, and supports legacy boolean comparators with one deprecation and the reversed-operand fallback. The full array audit reaches 807/842, an exact +6/-0 delta, while Zend/lang has no status change. All five feature configurations, unsafe, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 pass; startup, scalar-Long and impure callback release lanes remain below the +5% median ceiling. The remaining 1,116 Zend/lang failures, 21 array-suite failures and broader PHP compatibility remain explicit work. |
