...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5 AMD64 checkpoint is now 4,097/5,599 with zero timeouts and zero crashes. A shared cold scanner supplies `strip_tags()`, `highlight_string()`, `highlight_file()`/`show_source()` and `php_strip_whitespace()` plus the five exercised `highlight.*` CLI-INI directives. The exact delta is +9/-0 over `e861099a`, with every prior pass preserved and two byte-identical full manifests; one further test advances only to its independent `eval()` mismatch. Nine of ten focused Zend PHPTs and 21 of 44 adjacent `ext/standard` samples pass. All five feature configurations, the unchanged unsafe ratchet, Composer S0, Symfony S1 and PHP 8.5.9 S2/S3 gates pass; the paired startup median changes by -3.634%, below the +5% gate. Complete lexer fidelity, remote/custom wrappers, broader path diagnostics, every malformed `strip_tags()` state, sanitizer suitability, the `eval()` blocker and broader compatibility remain explicit work. |
