...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 4,077/5,599 with zero timeouts and zero crashes. `stristr()` implements binary-safe suffix/prefix selection with locale-independent ASCII folding, scalar/Stringable boundaries and exact null/NAN/error ordering. The exact delta is +1/-0 over `bc9bc620`, solely `gh12457.phpt`, with two byte-identical full manifests, every prior pass preserved and no other category movement; three adjacent unmodified standard-library PHPTs also pass. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; paired release changes are +0.327% for 100 empty requests and +1.392% for two million existing `strstr()` calls, below the +5% gate. Unicode folding, `md5()`, detached strict-call metadata and broader PHP 8.5 clusters remain explicit work. |
