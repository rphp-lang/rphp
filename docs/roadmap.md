...
| --- | --- | --- | --- |
| PHP compatibility | [Compatibility roadmap](roadmap-compatibility.md) | [Compatibility Agent](agent-strategy-compatibility.md) | The pinned PHP 8.5.6 AMD64 checkpoint is now 3,429/5,599 with zero timeouts and zero crashes. `class_alias()` accepts internal classes and interfaces after original lookup and the shared reserved/`_` validation, preserving canonical identity and internal Reflection metadata; `ReflectionClass(alias)` exposes the definition's canonical name. The exact delta is +4/-0 over `459f8481`, with two byte-identical full manifests. All five feature configurations, unsafe ratchet and Composer/Symfony S0-S3 gates pass; no hot-path performance gate applies to these explicit cold builtins. The remaining alias output differences, eval diagnostics and other PHP 8.5 clusters stay explicit compatibility work. |
