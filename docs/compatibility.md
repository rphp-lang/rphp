...
subject values follow PHP's positional replacement schedule, including empty
search items, missing replacement items, cumulative count, subject-key
preservation and array/reference copy-on-write. Case-insensitive matching folds
ASCII only and leaves NUL, arbitrary high bytes and UTF-8 byte sequences intact.

The three public value parameters expose PHP 8.5's `array|string` types and the
optional fourth parameter is a real by-reference `count`; Reflection also
reports `array|string` returns. A handler-owned internal argument boundary
preserves those types without applying user-function coercion first. It
implements weak scalar, null-deprecation and stringable-object conversions,
strict rejection, the scalar-search/array-replacement error, resource/object
diagnostics and nested array-to-string warnings at PHP's observable stage.
Static, dynamic, named and `call_user_func_array()` paths share the contract.
Detached array callbacks retain the originating strict-types bit while ordinary
engine callbacks keep their established weak-call behavior.

Three original E2E regressions cover ordered multi-replacement, missing and
empty entries, associative keys, references and source immutability, binary and
case-insensitive bytes, nested arrays, weak/strict unions, Stringable and
invalid objects/resources, exact count writes and non-reference errors,
Reflection, and all admitted call shapes. Five independent clean-room probes
match the PHP oracle byte-for-byte; their output SHA-256 values are
`1338dbe716bc0233282923cc11acc7ad2d478f9c6cf0f8dcbbd34721bb870c0f`,
`259b56802807ad5545807680bc8a058a38c0eb84dbb7e4d9c1e12dd77b299923`,
`59bac9f206ec9833782e1b52f48dcf8b74aaf06d7892ca44e0a2c65649d55e20`,
`b99ac86ad19a820bb02d48380f9f38e912177072a0c3b5dce985d96bcbc39306`
and `9148deddcfd4e1cb7f9364fe04422c2942d6fde0293e4c275b470afeab6f23e9`.
The eight supplying cases move from 0/8 to 8/8; their baseline, PHP-oracle and
final-candidate normalized hashes are
`ee0324f24ae327b618b27ec47da05f562a8451c2a8e08b1bc1ac0aed40ed80d7`,
`101d31b234cc4b91d11fb65eb653282133304e6d47caffb9b38fdbbaf0f0205b`
and the same `101d31b234cc4b91d11fb65eb653282133304e6d47caffb9b38fdbbaf0f0205b`.

The complete 733-case strings audit moves from 479 to 487 passes, an exact
+8/-0 delta consisting only of the supplying cluster. There are 161 failures,
...
S0, all four Symfony S1 gates and PHP 8.5.9 S2/S3 pass. Production has 1,623
unsafe blocks, 289 unsafe functions, 361 SAFETY annotations and seven
...
with candidate
`ea952a8c9134c09bc6780101fc2a744a56f3c321332f586b43e47508bf0e3223`.
Paired medians are -26.620% for scalar `str_replace()`, -15.070% for scalar
`str_ireplace()` and -7.549% for array replacement. Adjacent controls are
-0.483% for `str_split()`, +1.256%, +0.271% and +2.130% for the three retained
`chunk_split()` shapes, and +2.192% for 200 empty processes. Every
output/checksum matches and every median is below +5%.

This checkpoint does not claim locale-dependent non-ASCII case folding, exact
partial-count mutation after a throwing nested conversion, source-unpack
diagnostic locations, 32-bit execution or the remaining strings/Zend failures.
The next bounded high-yield candidate is the four visible `substr_replace()`
cases, which can reuse the proven PHP-byte, array-key, reference and typed-call
boundaries while adding offset/length broadcasting and splice semantics.
...
`htmlspecialchars()`. UTF-8 literals receive U+FFFD where the selected document
forbids them, while invalid UTF-8 without `ENT_SUBSTITUTE` remains an empty
result. `double_encode=false` now preserves numeric references only when their
bounded value and selected document/flag combination admits them; this includes
PHP 8.5's observable distinction between literal HTML5 carriage returns and
their numeric references.

The same engine decodes every admitted single-byte encoding before selecting
named entities or applying the document rules. A disallowed character that the
target encoding cannot represent becomes `&#xFFFD;`, while the basic
`htmlspecialchars()` path keeps PHP's byte-oriented high-byte behavior. The
existing SJIS/Shift_JIS/CP932 basic-only path now validates single-byte,
half-width and lead/trail units, produces the PHP notice from `htmlentities()`
and rejects incomplete or invalid units. The reversible Encoding Standard
tables now provide both encode and decode directions. No dependency, opcode,
runtime layout field or unsafe block was added.

Three original E2E regressions cover literal C0/C1/noncharacter boundaries,
document-specific numeric-reference preservation, Windows-1251 named entities,
unrepresentable replacements, handled SJIS diagnostics and invalid units. The
supplying `htmlentities20.phpt` through `htmlentities22.phpt` family moves from
0/3 to 3/3. Its normalized baseline, PHP-oracle and final-candidate SHA-256
values are
`16f4d90567c9e1c173e6edfdc58bd671b18433314553a2f24625da90c329e0fc`,
`0f3df0c330585ca1ca4553be9f3c1386aec4b429ad79763e45649b8a92e37bfc`
and `0f3df0c330585ca1ca4553be9f3c1386aec4b429ad79763e45649b8a92e37bfc`.

The complete 733-case strings audit moves from 424 to 427 passes, an exact
+3/-0 delta comprising only the supplying family; 221 failures, 54 skips, 30
unsupported cases, the retained `dirname_multi.phpt` timeout and zero crashes
remain. Array stays byte-for-byte equal to its parent at 828 passes, 13 skips
and one unsupported case. Zend/lang also stays byte-for-byte equal at 4,202
passes, 1,101 failures, 115 skips and 181 unsupported cases with no timeout or
crash. Three serial final runs produce identical normalized
path/status/category SHA-256 values
...
Symfony S1 gates and PHP 8.5.9 S2/S3 pass. Production remains at 1,621 unsafe
blocks, 289 unsafe functions, 357 SAFETY annotations and seven `# Safety`
sections. Exact-binary CPU-2-pinned balanced release A/B controls compare parent
SHA-256
`966bb5d9e0f0f2764f77a24879f10547f41417908cccf57c0b1eb57d50e03f6b`
with candidate
`9a9730ee690b8624eeeaa2627c007c7b7201ed0fc76f820269045200a1a92753`.
Paired medians are -2.570% for 500,000 default ASCII entity calls, -4.111%
for 500,000 paired `double_encode=false` entity/special-character iterations,
+2.101% for one million default decode calls, +0.572% for five million ordinary
concatenations and -0.244% for 1,000 empty-process pairs. Independent medians
...
`.5e2`, `.5e-2` and `.1_2` match PHP 8.5.9. The longer `...` and `.=` tokens
retain precedence, `1..5` remains two adjacent floats and therefore a parse
error, and a separated `1 . 5` remains string concatenation.

One lexer unit test and one original arithmetic E2E test cover numeric values,
unary signs, exponents, digit separators, ambiguous adjacency and the
concatenation boundary. The complete 842-case `ext/standard/tests/array` audit
moves from 693 to 717 passes (+24/-0), with 111 failures, 13 skips, one
unsupported case and no timeout or crash. All 58 source-confirmed leading-dot
cases advance beyond their former parse error: 24 become exact passes and the
other 34 remain visible output failures in independently incomplete array
comparison/callback families. Two serial final manifests and summaries are
respectively byte-identical with SHA-256
`4a91db412cdb5c6dd55d730d2b009fa070bcf4f9b4d41ff14795b040e81014eb`
and `e5df0d858ba2ca0a4eee4ab412e784c7e25d0a26c1fde6ab3e28ee43662d937f`.

The separate 5,599-case `Zend/tests` and `tests/lang` gate remains exactly
4,171 pass, 1,132 fail, 115 skip, 181 unsupported and zero XFAIL, timeout or
crash (+0/-0). Both candidate manifests are also byte-identical to the
preceding baseline. The two final manifest and summary SHA-256 values are
`fe8ee9e31f8b441d8b84198f1edc7f874c1b48a6bb6355911f58da4756ab82c8`
and `d71bd5280e745b2294b1fb5bee06862127ea8a2ca0c51d8b97ffe29ffcec6990`.
...
The 34 advanced output failures still depend on independent `array_diff*`,
`array_intersect*`, user-comparator, recursive merge and multisort semantics.
The remaining 111 array-suite failures, 1,132 Zend/lang failures and broader
front-end and PHP compatibility remain explicit follow-up work.
...
181 are unsupported, and none time out or crash. The exact pass-set delta from
`7e733ca2` is +6/-0 through `Zend/tests/bug46106.phpt`, closures 042, 065, 067
and 068, and dynamic invocation of a reference-returning function. Two final
runs have the same manifest SHA-256,
`ca4d609a84a40620616fa10dda3ea95b790a9cafbe7a322dd0f51832f5b0dfbe`,
and summary SHA-256,
...
original closure name and `getClosureScopeClass()` state. Public
`ReflectionClass` debug projection and the dummy `Closure` scope produced by
object-only binding complete the admitted metadata slice.

The 14-case unmodified PHP 8.5 focus moves from one to 12 passes, an exact
+11/-0 delta with no timeout or crash; PHP 8.5.9 passes all 14. Its final RPHP
manifest/summary hash pair is
`fcfd749fa391f5a4a5c88d9dc692bc7cad8433ab8aaf150b7f198f9ea24a0f9c` /
`941cea625af527193572becdc88ef84f55daec9f31d586adc2ba5da113e887ca`;
the oracle pair is
`8d643f38da36815a5391bd5268df30de9f1120956ef8758ca3f28c7281fd5393` /
`659c6f28bd4b629c859b0344dd84435314efd916c036692a9c8e4fb5cf866569`.
The complete 493-case Reflection audit moves from 78 to 89 passes (+11/-0),
with 370 failures, 17 skips and 17 unsupported cases. Its final manifest/
summary pair is
`cf0c0a09d380f1c33d0dadd4bff696f81d456347739feaa27182fefefb9c5c9e` /
`d5853e9b062b4e58d58c1d378f83763b022260294dee93b3b665dcb23b20fbcb`.
The adjacent 163-case closure and dynamic-call audit moves from 95 to 100
passes (+5/-0), with 59 failures and four skips; its pair is
`e4f1d35a1a5fee2aab43627832db53a56b2cd0d8aebbff195b47e539bd238d24` /
`14056023b7598a79e29550635fff10fb9cabb5e14841358a885582b0fa323932`.

Five original E2E tests cover names and namespaces; absent, class and dummy
closure scopes; defaults, captures and bound receivers; named/by-reference
function arguments; and private, static, named and variadic method invocation.
The implementation adds no dependency, lockfile, opcode, `ExecuteData`, PHP
value/object/array layout or unsafe block/function. `Value` remains 16 bytes,
`PhpClosure` remains 80 bytes on AMD64 by using trailing padding, and
`CallPlan` remains four bytes by assigning its final spare flag to static-method
metadata. The production unsafe inventory remains at 1,623 blocks and 289
functions; two additional proof comments bring the SAFETY annotation count to
341.
...
S1 gates and PHP 8.5.9 warmed-kernel S2 and cold-build S3 pass. The first
general invocation path was rejected after the unchanged Reflection method
control regressed by about 96%. The accepted path retains the packed ABI for
wider or named calls and uses a validated raw fast path only for the common
narrow internal variadic call.

A local CPU-pinned balanced alternating AMD64 release comparison used 32 pairs
per lane, three warmups per binary, performance-governor CPU 2 and no excluded
samples. Independent/paired median changes are +0.110%/-0.006% for batches of
100 empty requests, -0.077%/-0.258% for two million closure calls,
+0.080%/+0.472% for 100,000 closure binds and +1.194%/+1.162% for 200,000
unchanged Reflection method invocations. Every lane stays below the +5% gate
with exact output. Baseline/candidate binary SHA-256 values are
`cfb3069fe3fd490548acd681af6567177070d1f0ef7754f9cb98a609030d98b7`
and `13fa22c7aa8d032f1858cd86279c70a5d5ed59760b5a9c1bb70a474e43a3da8c`.
The benchmark harness/log hash pair is
`fe416e0236a7514a8b945aaad82589dcf7143f4e64a16ee5a8373586fd17fc84` /
`b57ade26a0b336208ea9607a2d880f184ab3df5352f7493760a4c6cc522cefba`.

The two remaining focused failures precede the admitted Reflection behavior:
one requires general unknown-named-argument forwarding in `call_user_func()`
and one requires `DateTime`. Complete Reflection type/default/return metadata,
closure serialization, general named variadics and broader standard-library
compatibility remain separate work.
...
remain XFAIL, 181 are unsupported, and none time out or crash. The headline
pass rate is 77.484%, the whole-corpus rate is 73.388%, and 4,809 of 5,303
attempted cases reach runtime (90.685%). The exact delta from `494c1d19` is
+1/-0, `tests/lang/bug24652.phpt`. Two independently executed final runs have
the same manifest SHA-256,
`f5dcfce168946156c41386bae89610bc8eff74387027c549665489610d9d0934`,
and the same summary SHA-256,
`9b5b62a70acbf3b8636d7d2d259a493479ce4fc8efa8ea73b21294883375c5dc`.

`in_array()`, `array_search()`, `array_key_exists()`/`key_exists()`,
`array_sum()` and `array_product()` now cover their exercised PHP 8.5
signatures, typed arguments, weak and strict conversion, canonical keys,
insertion-order search results, loose and recursive strict comparison,
references/COW, warnings and throwing handlers, sequential numeric-kind
transitions and integer-overflow promotion. Lookup comparison retains exact
integer semantics beyond the IEEE-754 exact range. Recursive lookup and
`array_keys()` comparison raise PHP's catchable nesting error, and runtime-CV
commutative operator diagnostics preserve PHP's source-dependent operand
order. The implementation is independent and does not copy or mechanically
translate php-src code, tests, or an external named algorithm.

The 42-case unmodified PHP 8.5 focused cluster moves from 15 to 31 passes, an
exact +16/-0 pass-set delta; PHP 8.5.9 records 37 passes, one local FFI
configuration failure and four extension skips. The complete recursive
842-case `ext/standard/tests/array` audit moves from 546 to 564 passes with no
lost pass: failures fall from 282 to 264 while 13 skips, one unsupported case,
zero timeouts and zero crashes remain. Two adjacent bug regressions pass
outside the focused cluster. Two final focused and full-array runs are
byte-identical. Their manifest/summary SHA-256 pairs are respectively
`9cc4635f69c1b01ce2d9df6496e05a479a6b44f4d45abb273da771b7e18969f8` /
`36c520d5313aa2d9b19ab3ecdded4ead38de3d832dc9bc9500419ddd10b49963`
and
`141a662ec719751869ff44a37cfcf1588b0e81b6cac9e494d8702a5aa3433210` /
`6c8414c0e7fe98d5237986637b2739faa7e0d55a2462198a7bbbffb422a6db0a`.

Five original E2E tests cover Reflection metadata, loose/strict lookup,
canonical keys, reentrant diagnostics, recursive arrays, references,
large numeric strings, aggregate warnings, handler exceptions and overflow.
Their five embedded programs produce the same 3,503 output bytes under RPHP
and PHP 8.5.9, SHA-256
`90301cb77827a2814b1a851b330b248de83a99a7481efcdf45d910532fe4c9c5`.
All five Cargo feature configurations, all-feature/all-target, formatting,
PHPT-runner and unsafe-policy self-tests, the unchanged 1,623-block/289-
function unsafe ratchet with 337 SAFETY annotations, Composer 2.8.12 S0, all
four Symfony S1 gates and PHP 8.5.9 warmed-kernel S2 and cold-build S3 pass.
...
A local CPU-pinned 32-pair balanced alternating AMD64 release comparison on
an AMD Ryzen 9 7950X used performance-governor CPU 2, three warmups per binary
and no excluded samples. Paired median changes are +0.072% for batches of 150
empty requests, -51.410% for strict lookup, -15.351% for loose lookup, +0.313%
for key existence, -11.331% for integer sum, -13.097% for integer product,
-0.591% for double sum and -0.464% for the scalar-comparison control. Every
comparable workload remains below the +5% gate and exact outputs match.
...
`a14f9e1829002bc5b8a007fc698f3970602abf1aac2ae24cd6377dca3f6303ef`.
The separately disclosed mixed integer/double sum lane is +249.168% against a
baseline that does not implement PHP's sequential numeric-kind contract. It
is therefore not a like-for-like regression gate; a general optimized region
for that now-correct path is handed to the execution/performance workstream.

Five focused failures remain explicit. Three variation tests stop at
independent parser gaps and two require `opendir()`. Complete reflected
internal type/default/return metadata, the remaining array suite and broader
PHP compatibility remain separate work.
...
are unsupported, and none time out or crash. The headline pass rate remains
77.277%, the whole-corpus rate remains 73.192%, and 4,809 of 5,303 attempted
cases reach runtime (90.685%). The selected functions are exercised by a
separate `ext/standard` cluster, so the exact main-corpus pass-set delta from
`c8c43701` is intentionally +0/-0 with no status or category movement.
...
`dff002f63754267cbc2969e38fd8d60f691d5ede1dc2b7aea14209ce352d3224`
and `094a9238f083724cce82d6240ef79ac0b71bbffef06b9242acb11e9d48b24a07`.
...
`array_udiff()`, `array_udiff_assoc()` and `array_udiff_uassoc()` now perform
PHP 8.5 user-value difference, while `array_uintersect()`,
`array_uintersect_assoc()` and `array_uintersect_uassoc()` provide the matching
intersection family. Value-only variants compare values through the final
callback; `*_assoc()` requires an exact key as well; `*_uassoc()` uses the
penultimate callback for values and the final callback for keys. All six
preserve the first array's keys, insertion order, duplicate behavior and
PHP-visible reference lifetime. Callback validation precedes array validation
in the exercised PHP order, exceptions stop further comparisons, and
structural inputs are snapshotted before callback re-entry while referenced
cells remain live. Reflection names, variadic shapes and minimum arity match
PHP 8.5. `array_diff_key()` and `array_intersect_key()` were also corrected to
their one-required-array variadic PHP 8.5 signatures and one-array identities.

The 33 supplying unmodified PHP 8.5 `ext/standard` PHPTs move from 0/33 to
17/33 in both debug and release builds. Twelve remaining cases stop at
independent leading-dot numeric or binary-string-literal parser gaps, three
reach pre-existing `var_export()`/`print_r()` formatting gaps, and one reaches
a later independent three-array `array_intersect()` call after validating the
new user-comparator behavior. The PHP 8.5.9 oracle passes 33/33. A 300-result
clean-room matrix across all six functions, comparison-array counts, key/value
types and callback forms is byte-identical to PHP 8.5.9: 15,065 output bytes
with aggregate SHA-256
`9e242c156802e333aadbc111d39b86d45d1f786a8f4e72a31ddfc80921c070c9`.
Original E2E regressions cover comparison modes and duplicates, PHP's
small-array callback schedule, structural mutation and live aliases,
exceptions, validation precedence, Reflection metadata and runtime arity.

All five Cargo feature configurations, all-feature/all-target, formatting,
PHPT-runner and unsafe-policy self-tests, the unchanged 1,620/289/332 unsafe
ratchet, Composer 2.8.12 S0, all four Symfony S1 gates and PHP 8.5.9 warmed-
kernel S2 and cold-build S3 pass. The implementation extends one existing cold
stdlib module and changes registrations and tests; it changes no compiler or
opcode definition, call frame, PHP Value/object layout, dependency or
production unsafe code.

A local CPU-pinned 32-pair balanced alternating AMD64 release comparison on an
AMD Ryzen 9 7950X used performance-governor CPU 2, three warmups per binary
and no excluded samples. Batches of 100 empty requests measured baseline/
candidate medians of 214.357/214.104 milliseconds, -0.118% independently and
+0.137% paired, with paired p10/p90 changes of -1.626% and +2.183%. One
500,000-entry `array_diff_key()` request measured 664.627/661.021
milliseconds, -0.543% independently and +0.072% paired, with paired p10/p90
changes of -2.900% and +1.571%. The unchanged 500,000-call
`array_intersect()` control measured 447.269/436.872 milliseconds, -2.325%
independently and -2.046% paired, with paired p10/p90 changes of -3.729% and
-0.577%. Outputs were checksummed and every control remains below the +5%
gate. The baseline and candidate binary SHA-256 values are
`1c93db15a866e6101db3eeeea07e700030cda05ac295161b2d84f8369a021788`
and `6f6138cec707dd6f9d7f804560a4f16511dd1100864508c3e196f3e98b2d426a`.

An attempted prerequisite migration of ordinary `array_intersect()` to its
PHP 8.5 variadic signature was rejected and reverted: variadic argument
packing first regressed the 500,000-call control by about 154%, and a scalar
fast path still regressed it by about 20%. This checkpoint therefore makes no
claim for three-or-more-array ordinary `array_intersect()`. For six or more
entries, user comparison uses a deterministic stable O(n log n) merge
baseline whose results match the exercised oracle; exact PHP comparator call
traces are claimed only for the exercised two-to-five-entry schedules, not for
arbitrarily large or inconsistent-comparator inputs. The 16 independent PHPT
blockers, `array_change_key_case()`, ordinary variadic `array_intersect()`, the
complete `ext/standard` array suite and broader PHP compatibility remain
separate work. The unchanged main-corpus count is not presented as a Zend/lang
pass gain.
