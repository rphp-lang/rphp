...
## Symfony first-application lifecycle gate
...
S2 checkpoint (2026-08-12): the pinned FrameworkBundle 7.4.16 production
fixture now passes with a cache generated by reference PHP. Its unmodified
Composer autoloader, generated container, compiled route matcher, event
dispatcher, controller resolver and HttpFoundation request/response path
handle one synthetic GET `/health` and produce `200|warmed|OK`; termination and
kernel shutdown also complete. The reproducible script performs a clean locked
install with checksum-pinned Composer 2.8.12, warms under reference PHP,
compares both runtimes and rejects cache mutation by RPHP. This completes the
warmed diagnostic only. The roadmap now advances to S3: RPHP must cold-build,
publish, reload and reuse its own container and route cache before the first
Symfony compatibility claim is made.
