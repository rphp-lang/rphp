...
#### Symfony warmed FrameworkBundle kernel S2 gate (2026-08-12)

The S2 diagnostic now passes. The fixture pins `symfony/framework-bundle`
v7.4.16 and its complete `composer.lock`; checksum-pinned Composer 2.8.12
performs a clean install in a task-scoped temporary directory. Reference PHP
boots a `prod`, `debug=false` kernel to generate the full container and
compiled route cache. The same unmodified autoloader and warmed files then run
under RPHP, handling a synthetic GET `/health`, termination and shutdown. Both
runtimes must print exactly `200|warmed|OK`, and a cache-tree digest proves that
RPHP did not rewrite the reference-built artifacts. The local entry point is
`scripts/test-symfony-warmed-kernel-s2.sh`; CI owns the matching
`Symfony warmed kernel S2` job.

The compatibility work closes general PHP/runtime paths reached by the real
kernel: anonymous classes with promoted properties and methods, callable
objects, first-class callable reflection, parameter/type metadata, eager
fallback semantics for native lazy ghosts, namespaced built-in fallback,
call-frame argument introspection, error logging and the core
`SplObjectStorage` type identity. Focused E2E regressions retain these paths
independently of Symfony. Attribute syntax is accepted but discarded, so
Reflection attribute queries remain truthfully empty in this fixture; broader
attribute semantics remain an S3 blocker.

The Linux portability closure adds the remaining paths exposed by the exact
CI environment: reference-return declarations remain loadable, expression
destructuring preserves its result, dynamic property `isset()`/`unset()` works,
same-class constants resolve in property defaults, conditional declarations
select only their live constant branch, class aliases expose effective
trait/inherited methods, `static` returns satisfy `object`, and the exercised
ReflectionClass/ReflectionProperty metadata and constructor-free allocation
surface is present. A follow-up hot-path audit keeps `substr_compare()`
allocation-free, avoids redundant destructuring temporaries and limits the
recursive generator-marker scan to constant-folded branches.

This is still the roadmap's localization diagnostic, not the first Symfony
compatibility claim. The container and routes were generated by reference PHP,
the request is in-memory and short-lived, native lazy ghosts are initialized
eagerly, and no repeated-worker or HTTP adapter contract is implied. Work now
advances to S3, where RPHP itself must cold-build, atomically publish, reload
