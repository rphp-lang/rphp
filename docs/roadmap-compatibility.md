...
The `core-output-handler-startup` checkpoint over `daa98eb0` validates and
initializes real CLI output handlers before parsing/user declarations. It
preserves startup diagnostics, read-only configuration, nested buffers, exit
finalization and pending exceptions. Global exception handlers observe the
failed final buffer without re-entering its callback or mutating its stack.

The complete 7,174-case stable core is
**6,850 pass / 4 fail / 192 skip / 127 unsupported / 1 timeout / 0 crash**,
exact **+7/-0**. Zend/lang stays **5,365/4/116/113 plus 1 timeout**;
strings/array is **1,485/0/76/14**. Fifteen unchanged output-handler PHPTs become
seven passes and eight genuine extension/locale skips. These are admission
gains, not htmlentities implementation fixes; both binaries agree with the new
runner. Every earlier pass and unrelated status/failure stage is preserved.
The known `new_oom.phpt` timeout and four library failures remain visible.

The slice adds 65 original CLI regressions: 64 are PHP 8.5.11 byte-exact and
one asserts only startup-before-parse ordering. Focused/adjacent Cargo covers
159 tests; all 63 inline release-oracle specimens are byte-exact. Five Cargo configurations,
...
PHP compatibility: four library failures, 127 unsupported cases and the OOM
timeout remain. The next data-selected admission candidate is diagnostic policy
(repeated errors, fatal backtraces, display/logging and HTML output). Baseline
the unchanged core and adjacent cases before implementation; do not count
extension skips as passes. General malformed INI syntax, parser diagnostic
spelling and library output-introspection APIs remain explicit non-claims.
...
### Preceding trait-constant checkpoint

The `core-trait-constant-comparison` checkpoint over `c0adfe49` reached 6,843
stable passes and four library failures, exact +1/-0, through deferred trait
collision comparison. Reference-crashing parent-relative forms remain an
explicit oracle holdout.
...
admission, thresholds, reference/COW ownership and destructor-worker shutdown.
...
exact passes without loss, reaching 6,771 passes and 11 failures. It preserved
generator-finally throws, reference-foreach cursor state and lazy property
snapshots.
...
destructor-created roots and completed failed property inc/dec writes.
...
passes without loss, reaching 6,762 passes and 20 failures. It aligned GC
...
The `lvalue-error-order-remainder` checkpoint over `7355a04d` reduced the
stable-core debt from 53 to 47, exact +6/-0, by aligning lvalue error ordering,
include/eval symbol updates and typed-reference return validation.
...
The `front-end-source-declaration` checkpoint over `be54b74b` reduced the
stable-core debt from 73 to 63, exact +10/-0. It aligned labels, delimiter and
visibility diagnostics, heredoc/nowdoc origins, eager declaration timing and
the callable `clone()` surface.
...
The `exception-handler-finalization` checkpoint over `df1f54f8` reduced
supported failure debt from 165 to 155, exact +10/-0. It aligned replacement
exception handlers, shutdown lifecycle, chunked output-handler failures,
fatal error state, callback traces and headers-sent origins.
...
Its ten-case native-stream argument admission became the current checkpoint.
Resource-first diagnostics reuse checked argument helpers without adding a
second successful resource lookup. Exact evidence and bounded non-claims
remain in `compatibility.md`.
...
existing stat/lstat cache and wrapper policy; `fstat` preserves backend-specific
native, standard, memory, temporary and user-wrapper behavior without adding
common stream-I/O work. Eleven original differential E2E cases and the focused
upstream projection produce 23 exact gains with no loss; four extension-loader
cases remain explicitly unsupported. The fixed-parent 7,174-case core remains
byte-identical at 6,390 exact passes, 390 supported failures, 182 skips and 212
unsupported cases, with no timeout or crash.

The builtin inventory now reports 489 of 1,201 reference globals present and
712 missing; the Symfony-base projection reports 487 present and 246 missing.
Every newly admitted function is call-shape and metadata exact. Five Cargo
configurations, all-target, exact upstream no-loss, Composer/Symfony S0-S3,
format and the unchanged 1,623/289 unsafe ratchet pass. The final fixed-parent
CPU-2 32-pair packet keeps every common lane neutral or faster except
file-stream reads at +0.943%; the changed `fileperms` pay-use lane improves by
82.265%. Current work remains selected as a 10–30-case train around one shared
...
The `97d26be4` `nested-ref-foreach-mutation` checkpoint then moves Zend/lang
from 4,441 to 4,450 pass, exactly `+9/-0`. Direct array `unset()` reports the
removed bucket's ordered position to the common live-reference cursor
translator, which adjusts each matching nested or caller-frame loop
independently. Current and previous deletion no longer skip a survivor;
future/next deletion, insertion, append, COW, final aliases and existing
internal shift/splice behavior remain stable.
...
CPU-2-pinned 32-pair release gate places all comparable array/foreach paths
between -2.913% and +1.930%, with matching outputs. Current-bucket deletion
...
The `b68a1505` `ordinary-abstract-method-compile-fatal-staging` checkpoint then
moves Zend/lang from 4,491 to 4,493 pass, exactly `+2/-0`. A concrete class
that directly declares an ordinary abstract method remains valid grammar and
fails at compilation with the namespace-qualified class, first abstract method
and method line. Normal classes validate after name/import checks; only
constant-elided and post-return source regions use the recursive declaration
preflight, preserving parser/deferred-error and source-order priority. A
runtime-compiled include/eval compile fatal is uncatchable and preserves prior
output, shutdown state, source metadata and the selected `display_errors`
presentation profile. Trait, interface, abstract class and property-hook
...
The `f4d2fa4c` `contextual-enum-identifier-grammar` checkpoint then moves
Zend/lang from 4,496 to 4,498 pass, exactly `+2/-0`. `enum` retains its real
declaration prefix while becoming a source-spelled contextual identifier for
PHP's namespace, classlike, function, constant, member, type, import,
qualified-name and label positions. Statement disambiguation preserves actual
and malformed enum declarations as well as later-syntax-error priority. The
targeted gains are `enum-reserved-non-modifiers.phpt` and
`keyword-no-bc-break.phpt`; the remaining 150 enum outcomes do not move.
...
S0, four Symfony S1 gates and PHP 8.5.9 S2/S3 pass. Production remains at the
unsafe ceiling of 1,623 blocks and 289 functions. One CPU-2-pinned 32-pair
parser/compiler gate places affected and control paired medians between
-0.583% and +1.644%. A 64-pair control of 100 empty requests through
...
The `f76fe749` `user-enum-reflection-property-projection` checkpoint then
moves Zend/lang from 4,509 to 4,510 pass, exactly `+1/-0`. Engine-created
`ReflectionProperty` wrappers expose only their public readonly `name` and
`class` strings in canonical order; target, modifiers, type and default state
move to a weak-owner request-local sidecar. Unit/backed enum handles and
synthetic case reads remain exact, while ordinary inherited, static, readonly,
typed and dynamic-property reflection methods preserve their state. Debug,
object-vars, JSON, clone and wire operations now follow the same public
projection and forbidden-object boundaries. The only enum movement is
`enum/name-property.phpt`; the other 151 outcomes are stable.

Two serial exact-final-binary Zend/lang runs have identical raw manifests,
path/status/category projections and pass sets; two strings/array projections
also match the exact parent. The adjacent 493-case ext/reflection directory
gains four passes without a lost parent pass. All five Cargo configurations,
all-feature/all-target, formatting, HTML data, PHPT runner, unsafe, Composer
S0, four Symfony S1 gates and PHP 8.5.9 S2/S3 pass. Production remains at the
unsafe ceiling of 1,623 blocks and 289 functions. Two CPU-2-pinned 32-pair
release gates keep startup, ordinary-property read, enum-property read and
ordinary-object print below the +5% ceiling; reflected construction and
enumeration improve in both runs, while wrapper printing remains a separately
reported changed-output path.

The monitored supported debt is now 811 failures: 18 strings, zero array and
793 Zend/lang. The two remaining enum failures are the AST dumper and
