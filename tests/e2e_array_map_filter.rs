...
fn test_array_map_undefined_callback_error() {
    ...
    let err = common::run_php_expect_error(r#"<?php
...
"#);
    match err {
        rphp::vm::execute::VmError::Fatal(msg) => {
            assert!(msg.contains("undefined"), "Error should mention undefined: {}", msg);
        }
        other => panic!("Expected Fatal, got {:?}", other),
    }
}
...
"#);
    let msg = format!("{:?}", err);
    assert!(msg.contains("nonexistent"), "expected function name in fatal: {}", msg);
}
...
"#);
    ...
    assert!(out.contains("nonexistent"), "Expected function name: {}", out);
}
...
"#);
    let msg = format!("{:?}", err);
    assert!(msg.contains("nope"), "expected function name in fatal: {}", msg);
}
