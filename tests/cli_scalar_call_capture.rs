...
#[test]
fn published_plan_limits_preserve_warm_calls_and_canonical_fallbacks() {
    for disable_jit in [false, true] {
        let mut command = std::process::Command::new(env!("CARGO_BIN_EXE_rphp"));
        command.arg(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/tests/fixtures/scalar_call_shape_limits.php"
        ));
        if disable_jit {
            command.env("RPHP_DISABLE_JIT", "1");
        }
        ...
        assert_eq!(
            output.stdout,
            include_bytes!("fixtures/scalar_call_shape_limits.out")
        );
    }
}
