...
fn test_e2e_nested_ternary_error() {
    let tokens = Lexer::new("<?php echo 1 ? 2 : 3 ? 4 : 5;").tokenize().unwrap();
...
fn test_e2e_dot_assign() {
    assert_eq!(run_php("<?php $x = \"hello\"; $x .= \" world\"; echo $x;"), "hello world");
}
...
fn test_identical_arrays_equal() {
    assert_eq!(run_php("<?php echo [1, 2, 3] === [1, 2, 3] ? 'yes' : 'no';"), "yes");
}

#[test]
fn test_identical_arrays_different_values() {
    assert_eq!(run_php("<?php echo [1, 2] === [1, 3] ? 'yes' : 'no';"), "no");
}

#[test]
fn test_identical_arrays_different_length() {
    assert_eq!(run_php("<?php echo [1, 2] === [1, 2, 3] ? 'yes' : 'no';"), "no");
}

#[test]
fn test_identical_arrays_different_keys() {
    assert_eq!(run_php("<?php echo ['a' => 1] === ['b' => 1] ? 'yes' : 'no';"), "no");
}
...
fn test_identical_with_concat_rhs() {
    assert_eq!(run_php("<?php echo 'xy' === 'x' . 'y' ? 'yes' : 'no';"), "yes");
}
