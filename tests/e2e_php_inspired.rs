...
fn test_php_func_return_as_if_condition() {
    ...
    assert_eq!(
        run_php("<?php function check($a) { if ($a < 3) { return 3; } return 0; } $a = 1; if ($a < check($a)) { echo $a; }"),
        "1"
    );
}
...
fn test_php_func_return_controls_loop() {
    ...
    assert_eq!(
        run_php("<?php function limit() { return 5; } $i = 0; while ($i < limit()) { echo $i; $i++; }"),
        "01234"
    );
}
...
fn test_php_nested_switch_in_function() {
    ...
    assert_eq!(
        run_php("<?php function classify($n) { switch ($n) { case 0: return 'z'; case 1: return 'o'; default: return 'd'; } } for ($i = 0; $i < 4; $i++) { echo classify($i); }"),
        "zodd"
    );
}
...
fn test_php_switch_with_nested_switch_in_function() {
    ...
    assert_eq!(
        run_php("<?php function test($i, $j) { switch ($i) { case 0: switch ($j) { case 0: echo 'a'; break; case 1: echo 'b'; break; default: echo 'c'; break; } break; default: echo 'x'; break; } } test(0, 0); test(0, 1); test(0, 5); test(1, 0);"),
        "abcx"
    );
}
...
fn test_php_recursive_with_while() {
    ...
    assert_eq!(
        run_php("<?php function countdown($n) { if ($n <= 0) { return 0; } echo $n; $j = $n - 1; while ($j > $n - 2) { countdown($j); $j = $j - 1; } return 0; } countdown(3);"),
        "321"
    );
}
...
fn test_php_recursive_sum() {
    ...
    assert_eq!(
        run_php("<?php function rsum($n) { if ($n <= 0) { return 0; } return $n + rsum($n - 1); } echo rsum(10);"),
        "55"
    );
}
...
fn test_php_early_return_in_loop_call() {
    ...
    assert_eq!(
        run_php("<?php function maybe($n) { if ($n % 2 == 0) { return $n; } return 0; } $sum = 0; for ($i = 0; $i < 6; $i++) { $sum += maybe($i); } echo $sum;"),
        "6"
    );
}
...
fn test_php_switch_in_for_all_branches() {
    ...
    assert_eq!(
        run_php("<?php for ($i = 0; $i <= 3; $i++) { switch ($i) { case 0: echo 'A'; break; case 1: echo 'B'; break; case 2: echo 'C'; break; default: echo 'D'; break; } }"),
        "ABCD"
    );
}
...
fn test_php_switch_fall_through_chain() {
    ...
    assert_eq!(
        run_php("<?php $x = 1; switch ($x) { case 1: echo '1'; case 2: echo '2'; case 3: echo '3'; break; case 4: echo '4'; }"),
        "123"
    );
}
...
fn test_php_switch_string_comparison() {
    ...
    assert_eq!(
        run_php("<?php $s = 'banana'; switch ($s) { case 'apple': echo 'A'; break; case 'banana': echo 'B'; break; case 'cherry': echo 'C'; break; default: echo 'X'; }"),
        "B"
    );
}
...
fn test_php_nested_while_multiplication_table() {
    ...
    assert_eq!(
        run_php("<?php $i = 1; while ($i <= 3) { $j = 1; while ($j <= 3) { echo $i * $j; $j++; } $i++; }"),
        "123246369"
    );
}
...
fn test_php_nested_if_two_vars() {
    assert_eq!(
        run_php("<?php $a = 1; $b = 2; if ($a == 1) { if ($b == 1) { echo 'aa'; } elseif ($b == 2) { echo 'ab'; } else { echo 'ax'; } } else { echo 'no'; }"),
        "ab"
    );
}
...
fn test_php_elseif_chain_exhaustive() {
    ...
    assert_eq!(
        run_php("<?php for ($i = 0; $i < 5; $i++) { if ($i == 0) { echo 'A'; } elseif ($i == 1) { echo 'B'; } elseif ($i == 2) { echo 'C'; } elseif ($i == 3) { echo 'D'; } else { echo 'E'; } }"),
        "ABCDE"
    );
}
...
fn test_php_function_with_loop_and_switch() {
    ...
    assert_eq!(
        run_php("<?php function fizz($n) { $r = ''; for ($i = 1; $i <= $n; $i++) { switch ($i % 3) { case 0: $r .= 'F'; break; default: $r .= $i; break; } } return $r; } echo fizz(6);"),
        "12F45F"
    );
}
...
fn test_php_recursive_fibonacci() {
    assert_eq!(
        run_php("<?php function fib($n) { if ($n <= 1) { return $n; } return fib($n - 1) + fib($n - 2); } echo fib(10);"),
        "55"
    );
}
...
fn test_php_loop_with_ternary() {
    assert_eq!(
        run_php("<?php $r = ''; for ($i = 0; $i < 6; $i++) { $r .= ($i % 2 == 0) ? 'E' : 'O'; } echo $r;"),
        "EOEOEO"
    );
}
...
fn test_php_array_reverse_manual() {
    ...
    assert_eq!(
        run_php("<?php $a = [10, 20, 30, 40, 50]; $r = ''; for ($i = 4; $i >= 0; $i--) { $r .= $a[$i]; } echo $r;"),
        "5040302010"
    );
}
...
fn test_php_array_swap_elements() {
    assert_eq!(
        run_php("<?php $a = [1, 2, 3]; $tmp = $a[0]; $a[0] = $a[2]; $a[2] = $tmp; echo $a[0]; echo $a[1]; echo $a[2];"),
        "321"
    );
}
...
fn test_php_array_as_lookup_table() {
    ...
    assert_eq!(
        run_php("<?php $t = ['a' => '1', 'b' => '2', 'c' => '3']; echo $t['b']; echo $t['a']; echo $t['c'];"),
        "213"
    );
}
...
fn test_php_function_modifies_array_copy() {
    ...
    assert_eq!(
        run_php("<?php function add_elem($arr) { $arr[] = 99; return $arr[0]; } $a = [42]; echo add_elem($a); echo $a[0];"),
        "4242"
    );
}
...
fn test_php_nested_array_build_and_read() {
    assert_eq!(
        run_php("<?php $matrix = []; for ($i = 0; $i < 3; $i++) { $row = []; for ($j = 0; $j < 3; $j++) { $row[] = $i * 3 + $j; } $matrix[] = $row; } echo $matrix[0][0]; echo $matrix[1][1]; echo $matrix[2][2];"),
        "048"
    );
}
...
fn test_php_break2_from_switch_in_loop_with_array() {
    ...
    assert_eq!(
        run_php("<?php $cmds = ['go', 'go', 'stop', 'go']; for ($i = 0; $i < 4; $i++) { switch ($cmds[$i]) { case 'stop': break 2; default: echo $i; } } echo 'end';"),
        "01end"
    );
}
...
fn test_php_bubble_sort() {
    // Simple bubble sort
    assert_eq!(
        run_php("<?php $a = [5, 3, 1, 4, 2]; for ($i = 0; $i < 5; $i++) { for ($j = 0; $j < 4; $j++) { if ($a[$j] > $a[$j + 1]) { $tmp = $a[$j]; $a[$j] = $a[$j + 1]; $a[$j + 1] = $tmp; } } } for ($i = 0; $i < 5; $i++) { echo $a[$i]; }"),
        "12345"
    );
}
